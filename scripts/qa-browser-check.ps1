...
const configTemplateAdvancedState = await evalValue('(() => { const details = document.querySelector(".config-template-console__details"); const detailsSummary = details ? [...details.children].find(child => child.tagName === "SUMMARY") : null; const defaultQuick = document.querySelector(".config-template-console__default-quick"); const defaultSummary = defaultQuick ? defaultQuick.querySelector("summary") : null; return { visible: Boolean(details), open: details ? details.open === true : null, summaryText: detailsSummary ? detailsSummary.innerText : "", defaultQuickPresent: Boolean(defaultQuick), defaultQuickOpen: defaultQuick ? defaultQuick.open === true : null, defaultSummaryText: defaultSummary ? defaultSummary.innerText : "", defaultActionCount: defaultQuick ? defaultQuick.querySelectorAll("button").length : 0 }; })()');
const configTemplateSourceState = await evalValue('(() => { const source = document.querySelector(".config-template-source"); return { visible: Boolean(source), text: source ? source.innerText : "", explainsActiveTemplate: Boolean(source && (source.innerText || "").includes("\u5f53\u524d\u751f\u6548\u6a21\u677f")), explainsFilteredChoices: Boolean(source && (source.innerText || "").includes("\u5df2\u7b5b\u9664\u4e0d\u5339\u914d\u6216\u7981\u7528\u6a21\u677f")) }; })()');
const configDensityState = await evalValue('(() => { const bodyFontSize = parseFloat(getComputedStyle(document.body).fontSize || "0"); const summary = document.querySelector("[data-config-density-summary]"); const assist = document.querySelector(".config-assist-drawer"); const editor = document.querySelector(".config-template-console"); const focusedEditor = document.querySelector(".editable-config-grid--focused"); const summaryRect = summary ? summary.getBoundingClientRect() : null; const editorRect = editor ? editor.getBoundingClientRect() : null; const focusedEditorRect = focusedEditor ? focusedEditor.getBoundingClientRect() : null; return { bodyFontSize, viewportHeight: window.innerHeight, summaryVisible: Boolean(summary), summaryHeight: summaryRect ? Math.round(summaryRect.height) : null, assistVisible: Boolean(assist), assistOpen: assist ? assist.open === true : null, editorVisible: Boolean(editor), editorTop: editorRect ? Math.round(editorRect.top) : null, focusedEditorVisible: Boolean(focusedEditor), focusedEditorTop: focusedEditorRect ? Math.round(focusedEditorRect.top) : null, focusedEditorBottom: focusedEditorRect ? Math.round(focusedEditorRect.bottom) : null, focusedEditorFieldCount: focusedEditor ? focusedEditor.querySelectorAll("input, textarea, select").length : 0 }; })()');
...
const result = {
  ...
  assertions: {
    ...
    configDefaultTemplatePackVisible: clickedConfig
      ...
      && configTemplateAdvancedState.visible === true
      && configTemplateAdvancedState.summaryText.includes(text.defaultTemplatePack)
      && configTemplateAdvancedState.defaultQuickPresent === true
      && configTemplateAdvancedState.defaultSummaryText.includes(text.defaultTemplatePack)
    ...
    configTemplateSourceVisible: clickedConfig
      && configTemplateSourceState.visible === true
      && configTemplateSourceState.explainsActiveTemplate === true
      && configTemplateSourceState.explainsFilteredChoices === true,
    ...
    configEditorNearFirstViewport: clickedConfig
      && configDensityState.focusedEditorVisible === true
      && configDensityState.focusedEditorTop !== null
      && configDensityState.focusedEditorFieldCount >= 1
      && configDensityState.focusedEditorTop <= Math.min(760, configDensityState.viewportHeight * 0.78),
    configPrecheckActionVisible: clickedConfig
      && configPrecheckState.visible === true
      && configPrecheckState.buttonVisible === true
      && (configPrecheckState.buttonText.includes('\u8fd0\u884c\u914d\u7f6e\u9884\u68c0') || configPrecheckState.buttonText.includes('\u5237\u65b0\u914d\u7f6e\u9884\u68c0'))
      && configPrecheckState.text.includes('\u4e0d\u4f1a\u64cd\u4f5c\u5e97\u5c0f\u79d8'),
    configDisabledReasonVisible: clickedConfig
      && (configPrecheckState.disabledReasonVisible === true || configPrecheckState.text.includes('\u914d\u7f6e\u9884\u68c0')),
    ...
    consoleInlineOperatorForms: consoleLoginFormDomState.hasForm === true
      ...
      && consoleLoginFormDomState.rememberCredential === true
      && consoleLoginFormDomState.credentialStateVisible === true
