...
WORKFLOW_READY_TERMS = {
    ...
    'editor': ['基本信息', '产品信息', '保存'],
    'semi_managed': ['半托管', '重量', '包装尺寸', '物流属性'],
}
WORKFLOW_PAGE_IDENTITIES = {
    'product': {
        'path': '/product/productList.htm',
    },
    'data_acquisition': {
        'path': '/web/productCrawl/dataAcquisition',
    },
    'draft_box': {
        'path': '/web/smt/smtProductList/draft',
        'query': {'status': '0'},
    },
    'editor': {
        'path': '/web/smt/edit',
        'required_query_keys': ('id',),
    },
    'semi_managed': {
        'path': '/web/smt/editFromSmt',
    },
}
...
class DxmLoginFlow:
    def __init__(self, live_client: DxmLiveClient, state_file: Path | None = None) -> None:
        ...
        self._mutation_authorizer: Callable[[dict[str, Any], Callable[[], Any]], Any] | None = None
        self._mutation_command_context: dict[str, Any] | None = None
        self._last_mutation_authorization_facts: dict[str, Any] | None = None
        self._execution_evidence_context: dict[str, Any] | None = None
        self._browser_session_generation: str | None = None
        self._browser_session_context_id: int | None = None
    ...
    def set_mutation_authorizer(
        self,
        authorizer: Callable[[dict[str, Any], Callable[[], Any]], Any] | None,
        command_context: dict[str, Any] | None = None,
    ) -> None:
        self._mutation_authorizer = authorizer if callable(authorizer) else None
        self._mutation_command_context = dict(command_context or {}) if self._mutation_authorizer else None

    def clear_mutation_authorizer(self) -> None:
        self._mutation_authorizer = None
        self._mutation_command_context = None

    def set_execution_evidence_context(self, context: dict[str, Any] | None) -> None:
        self._execution_evidence_context = dict(context or {}) if isinstance(context, dict) else None

    def clear_execution_evidence_context(self) -> None:
        self._execution_evidence_context = None

    @staticmethod
    def _safe_evidence_file_part(value: Any) -> str:
        text = str(value or '').strip()
        safe = ''.join(character if character.isalnum() or character in {'-', '_'} else '_' for character in text)
        return safe[:80]

    def _scoped_evidence_path(self, base_path: Path, *, action: str) -> Path:
        context = self._execution_evidence_context if isinstance(self._execution_evidence_context, dict) else {}
        parts = {
            key: self._safe_evidence_file_part(context.get(key))
            for key in ('task_id', 'job_id', 'state', 'command_id')
        }
        if not all(parts.values()):
            raise RuntimeError('EVIDENCE_CONTEXT_MISSING: 证据缺少 task/job/state/command 唯一上下文')
        action_part = self._safe_evidence_file_part(action)
        target = Path(base_path).resolve().with_name(
            f"{Path(base_path).stem}_task_{parts['task_id']}_job_{parts['job_id']}_{parts['state']}_{parts['command_id']}_{action_part}{Path(base_path).suffix}"
        )
        target.parent.mkdir(parents=True, exist_ok=True)
        return target

    @staticmethod
    def _evidence_descriptor(path: Path) -> dict[str, Any]:
        resolved = Path(path).resolve()
        if not resolved.is_file():
            raise RuntimeError(f'EVIDENCE_FILE_MISSING: {resolved}')
        content = resolved.read_bytes()
        if not content:
            raise RuntimeError(f'EVIDENCE_FILE_EMPTY: {resolved}')
        return {
            'path': str(resolved),
    ...
    def _capture_scoped_evidence_screenshot(
        ...
        *,
        ...
        target = self._scoped_evidence_path(base_path, action=action)
        try:
            descriptor = os.open(target, os.O_WRONLY | os.O_CREAT | os.O_EXCL)
        except FileExistsError as exc:
            raise RuntimeError(f'EVIDENCE_PATH_ALREADY_EXISTS: {target}') from exc
        temporary = target.with_name(f'.{target.stem}.{uuid.uuid4().hex}.tmp{target.suffix}')
        try:
            page.screenshot(path=str(temporary), full_page=full_page)
            content = temporary.read_bytes()
            if not content:
                raise RuntimeError(f'EVIDENCE_FILE_EMPTY: {temporary}')
            offset = 0
            ...
            try:
                ...
                pass
        return self._evidence_descriptor(target)

    def _dispatch_authorized_mutation(
        self,
        mutation_action: str,
        ...
        *,
        ...
        if authorizer is None:
            raise MutationAuthorizationError('AUTH_VERIFIER_MISSING: 真实写入点击缺少即时授权校验器')
        if not callable(operation):
            raise MutationAuthorizationError('MUTATION_OPERATION_INVALID: 真实写入 operation 不可调用')
        context = {
            **dict(self._mutation_command_context or {}),
            'mutation_action': str(mutation_action),
            '_pre_dispatch_guard': pre_dispatch_guard,
        }
        ...
        def guarded_operation() -> Any:
            nonlocal operation_attempts, operation_result, operation_error
            operation_attempts += 1
            if operation_attempts != 1:
                raise MutationAuthorizationError(
                    'MUTATION_OPERATION_MULTIPLE_EXECUTIONS: 同一授权 operation 禁止重复执行'
                )
            try:
        ...
        if operation_error is not None:
            raise MutationAuthorizationError(f'MUTATION_OPERATION_FAILED: {operation_error}') from operation_error
        if not isinstance(result, Mapping) or result.get('ok') is not True:
            ...
            raise MutationAuthorizationError(f'{reason_code}: 真实写入点击授权已失效')
        if operation_attempts != 1:
            reason_code = (
                'MUTATION_OPERATION_MULTIPLE_EXECUTIONS'
                if operation_attempts > 1
                else 'MUTATION_OPERATION_NOT_EXECUTED'
            )
            raise MutationAuthorizationError(f'{reason_code}: 授权回调没有 exactly-once 执行真实写入 operation')
        if result.get('executed') is not True:
            raise MutationAuthorizationError(
                'MUTATION_OPERATION_NOT_EXECUTED: 授权结果没有确认 operation 已执行'
            )
        if 'operation_result' not in result:
            raise MutationAuthorizationError(
                'MUTATION_OPERATION_RESULT_MISSING: 授权结果缺少 operation_result'
            )
        authorization_facts = {
            key: value
        ...
        self._last_mutation_authorization_facts = dict(authorization_facts)
        return {
            **dict(result),
            'ok': True,
            'executed': True,
            ...
            'authorization_facts': dict(authorization_facts),
        }

    @staticmethod
    def _run_mutation_pre_dispatch_guard(
        ...
        rejected = result is False or (
            isinstance(result, Mapping) and result.get('ok') is not True
        )
        if rejected:
            reason = (
                str(result.get('reason') or 'live mutation target changed')
                if isinstance(result, Mapping)
                else 'live mutation target changed'
            )
            raise MutationAuthorizationError(
                f'MUTATION_TARGET_DRIFT: {reason}'
            )

    def _bind_browser_context_generation(self, context: BrowserContext) -> str:
        browser = self._browser
        if (
            context is not self._context
            or browser is None
            or not callable(getattr(context, 'is_closed', None))
            or not callable(getattr(browser, 'is_connected', None))
            ...
            raise RuntimeError(
                'BROWSER_SESSION_OBJECT_INACTIVE: browser context must be current, open, and connected'
            )
        context_id = id(context)
        browser_id = id(browser)
        if (
            self._browser_session_context_id != context_id
            or self._browser_session_browser_id != browser_id
            or not self._browser_session_generation
        ):
            self._browser_session_generation = uuid.uuid4().hex
            self._browser_session_context_id = context_id
            self._browser_session_browser_id = browser_id
        if context_id not in self._browser_session_hooked_context_ids:
            on_context = getattr(context, 'on', None)
            if callable(on_context):
                try:
                    on_context('close', lambda *_args, bound=context: self._on_browser_context_closed(bound))
                    self._browser_session_hooked_context_ids.add(context_id)
                except Exception:
                    pass
        if id(browser) not in self._browser_session_hooked_browser_ids:
            on_browser = getattr(browser, 'on', None)
            if callable(on_browser):
                try:
                    on_browser('disconnected', lambda *_args, bound=browser: self._on_browser_disconnected(bound))
                    self._browser_session_hooked_browser_ids.add(id(browser))
                except Exception:
                    pass
        return self._browser_session_generation

    def _on_browser_context_closed(self, context: BrowserContext) -> None:
        self._browser_session_hooked_context_ids.discard(id(context))
        if self._browser_session_context_id == id(context):
            self._clear_browser_context_generation()

    def _on_browser_disconnected(self, browser: Browser) -> None:
        self._browser_session_hooked_browser_ids.discard(id(browser))
        if self._browser_session_browser_id == id(browser):
            self._clear_browser_context_generation()

    def _clear_browser_context_generation(self) -> None:
    ...
    def current_mutation_identity(self) -> dict[str, Any] | None:
        ...
        This method runs on the Browser Agent thread, so reading the current
        Playwright page is safe. The target hash is recomputed from task-bound
        values; claim/save click primitives separately revalidate the DOM target
        immediately before emitting their input event.
        """

        context = self._mutation_command_context
        page = self._page
        if not isinstance(context, dict) or page is None:
            return None
        page_url = str(getattr(page, 'url', '') or '').strip()
        expected_page = str(context.get('expected_page') or '').strip()
        if not page_url or not expected_page:
            return None
        identity_result = self._page_identity_result(page_url, expected_page)
        if identity_result.get('ok') is not True:
            return {
        ...
        command_action = str(context.get('command_action') or '').strip()
        target_values = context.get('mutation_target')
        ...
        return {
            ...
            'target_hash': observed_target_hash,
        }
    ...
    def _page_identity_result(self, page_url: str | None, expected_identity: str) -> dict[str, Any]:
        identity = WORKFLOW_PAGE_IDENTITIES.get(expected_identity)
        if not identity:
            return {
                'ok': False,
                'reason': 'unknown_expected_identity',
        ...
        try:
            ...
            parsed = urlparse('')
        expected_path = str(identity['path']).rstrip('/')
        actual_path = str(parsed.path or '').rstrip('/')
        host_ok = str(parsed.hostname or '').casefold() in {'dianxiaomi.com', 'www.dianxiaomi.com'}
        path_ok = actual_path == expected_path
        query = parse_qs(parsed.query)
        expected_query = identity.get('query') or {}
        query_ok = all((query.get(key) or [''])[0] == value for key, value in expected_query.items())
        required_query_keys = tuple(identity.get('required_query_keys') or ())
        required_query_ok = all(bool((query.get(key) or [''])[0]) for key in required_query_keys)
        return {
            'ok': bool(host_ok and path_ok and query_ok and required_query_ok),
            'reason': 'identity_match' if host_ok and path_ok and query_ok and required_query_ok else 'wrong_route',
            'expected_identity': expected_identity,
    ...
    def _browser_readiness_gate(
        self,
        page: Page,
        *,
        label: str,
        expected_identity: str,
        ready_terms: list[str] | tuple[str, ...] | None = None,
    ) -> dict[str, Any]:
        page_title = None
        try:
            title_method = getattr(page, 'title', None)
            if callable(title_method):
                page_title = str(title_method() or '')
        except Exception:
            page_title = None
        ...
        if self._page_looks_like_login(page):
            return {
                ...
                'page_url': page_url,
                'message': f'{label}页面显示为登录页，请重新登录店小秘后重试。',
            }
        identity_result = self._page_identity_result(page_url, expected_identity)
        if identity_result.get('ok') is not True:
            return {
                **identity_result,
                'ok': False,
                'reason': identity_result.get('reason') or 'wrong_route',
                'label': label,
                'requires_user_action': True,
                'page_title': page_title,
                'page_url': page_url,
                'message': f'{label}页面路由与期望页面不一致，已停止自动操作。',
            }
        state: dict[str, Any] = {
            'readyState': None,
            'loading': None,
        ...
        terms = [str(term) for term in (ready_terms or []) if str(term).strip()]
        try:
            evaluate = getattr(page, 'evaluate', None)
            if callable(evaluate):
                probed = evaluate(r'''(terms) => {
                  const textOf = (el) => (el?.innerText || el?.textContent || '').replace(/\s+/g, ' ').trim();
                  const visible = (el) => {
                    if (!el) return false;
                    const r = el.getBoundingClientRect();
                  ...
                  const loadingNodes = Array.from(document.querySelectorAll('.ant-spin-spinning, .vxe-loading, .vxe-loading--wrapper, .el-loading-mask, .ant-spin, .loading, [class*=loading], [class*=Loading]')).filter(visible);
                  ...
                  const markerNodes = Array.from(document.querySelectorAll(
                    'table thead th, .vxe-header--column, .ant-table-thead th, form label, input[placeholder], textarea[placeholder], button, [role=button], [role=tab], h1, h2, h3, .ant-tabs-tab, .el-tabs__item'
                  )).filter(visible);
                  const markerMatches = markerNodes
                    .map(el => textOf(el) || String(el.getAttribute('placeholder') || '').trim())
                  ...
                  return {
                    ...
                    businessMarker: markerMatches[0] || null,
                    businessMarkerCount: markerMatches.length,
                    bodyExcerpt: markerMatches.slice(0, 8).join(' | ').slice(0, 500),
                  };
                }''', terms)
            ...
            return {
                ...
                'page_url': page_url,
                'error': str(exc)[:240],
                'message': f'{label}页面状态检查无响应，请人工查看真实浏览器。',
            }
        if state.get('loading'):
            return {
                ...
                'page_url': page_url,
                'loading': True,
                'loading_count': state.get('loadingCount'),
                'body_excerpt': state.get('bodyExcerpt'),
                'message': f'{label}页面仍在加载，已停止自动操作并保留真实浏览器。',
            }
        if state.get('blockingModal'):
            return {
                'ok': False,
                ...
                'page_title': page_title,
                'page_url': page_url,
                'blocking_modal': state.get('blockingModal'),
                'message': f'{label}页面有弹窗或遮罩阻断，请人工处理后重试。',
            }
        body_excerpt = str(state.get('bodyExcerpt') or '')
        business_marker = str(state.get('businessMarker') or '').strip()
        if not business_marker:
            return {
                'ok': False,
                ...
                'expected_identity': expected_identity,
                'business_marker': None,
                'message': f'{label}页面没有出现目标页面关键内容，请人工确认页面是否打开正确。',
            }
        return {
            'ok': True,
            'reason': 'ready',
            ...
            'business_marker_count': state.get('businessMarkerCount'),
    ...
    def claim_from_data_acquisition(
        ...
        state = {
            ...
            'claim_dialog': result.get('claim_dialog'),
            'claim_click_safety': result.get('claim_click_safety'),
            'claim_result': dict(result),
        }
        self._write_state(state)
        return state
    ...
    def verify_draft_box_claim(
        ...
        if isinstance(result, dict) and result.get('ok') is False:
            state = {
                'ok': False,
                'stage': result.get('stage') or 'draft_box_claim_verify_failed',
                'label': result.get('label') or '商品箱确认失败',
                'message': result.get('message') or '商品箱商品确认没有完成。',
                'next_action': result.get('next_action') or '请打开商品箱检查认领商品是否出现，必要时回到已有待认领列表重新认领。',
                'requires_user_action': result.get('requires_user_action', True),
                'page_title': result.get('page_title'),
                'page_url': result.get('page_url'),
                'screenshot_url': result.get('screenshot_url'),
                'browser_visible': result.get('browser_visible', not self._is_headless()),
        ...
        verification_result = {
            **dict(result),
            'claim_mark': claim_mark,
            'claimed_product': result.get('claimed_product'),
            'draft_box_match': result.get('draft_box_match'),
            'claim_target': result.get('claim_target') or previous_claim_target,
            'search_result': (
                result.get('search_result')
                if result.get('search_result') is not None
                else previous_state.get('search_result')
            ),
        }
        state = {
            'ok': True,
    ...
    def perform_editor_action(
        ...
        state = {
            ...
            'unpublished_proof': (
                result.get('unpublished_proof') or result.get('fill_result')
    ...
    def _navigate_in_session(self, target: str) -> dict[str, Any]:
        ...
        if target == 'data_acquisition':
            ...
            if readiness.get('ok') is not True:
                raise RuntimeError(readiness.get('message') or '已有待认领列表页面未就绪')
            wait_result = {
                'ready': True,
                'ready_term': readiness.get('business_marker'),
                'loading': readiness.get('loading'),
                'rows': None,
                'inputs': None,
                'url': getattr(page, 'url', ''),
                'title': readiness.get('page_title') or '店小秘--数据采集',
                'text_excerpt': readiness.get('body_excerpt') or '',
                'expected_identity': 'data_acquisition',
                'readiness': readiness,
            }
            self._trace_workflow_event(
                ...
                target=target,
                current_url=getattr(page, 'url', None),
                reason='exact_identity_business_marker_loading_gate',
            )
            ...
            wait_result = self._settle_visible_draft_box(page, expected_identity='draft_box')
    ...
    def _perform_draft_box_action(
        ...
        if action == 'edit':
            ...
            if open_editor is not None:
                ...
                match = open_editor.get('match') or {}
                self._page = editor_page
                self._reapply_live_hud_if_available(editor_page)
                self._trace_workflow_event(
                    'draft_box_action:reuse_open_editor_for_target',
                    matched_by=match.get('matchedBy'),
                    source_urls=match.get('sourceUrls') or target_source_urls or [],
                    current_url=getattr(editor_page, 'url', None),
                    human_step='接管已打开的商品编辑页',
                )
                ...
                return {
                    'ok': True,
                    'page_title': page_title,
                    'page_url': getattr(editor_page, 'url', ''),
                    ...
                    'product_query': product_query,
                    'store_name': store_name,
                    'target_row_text': match.get('textExcerpt') or '已接管当前打开的编辑页',
                    'target_source_urls': match.get('sourceUrls') or target_source_urls or [],
                    'message': '已接管已打开的真实编辑界面。',
    ...
    def _perform_data_acquisition_claim(
        ...
        claim_click_receipt = self._click_data_acquisition_claim_rect_center(
            page,
            target['actionRect'],
            purpose='认领按钮',
            mutation_action='claim_open_dialog_click',
            pre_dispatch_guard=lambda: self._assert_data_acquisition_claim_click_safe(
                page,
                target,
                target_source_urls=target_source_urls or [],
            ),
    ...
    def _wait_for_data_acquisition_ready_for_claim(self, page: Page) -> dict[str, Any]:
        if os.name == 'nt' and not self._is_headless() and self._is_data_acquisition_page_url(page):
            return self._wait_for_visible_data_acquisition_claim_settle(page, expected_identity='data_acquisition')
    ...
    def _wait_for_visible_data_acquisition_claim_settle(self, page: Page, *, expected_identity: str) -> dict[str, Any]:
        identity_result = self._page_identity_result(getattr(page, 'url', None), expected_identity)
        if identity_result.get('ok') is not True:
            raise RuntimeError(
                f'已有待认领列表页面路由不匹配 expected_identity={expected_identity} '
    ...
    def _assert_data_acquisition_claim_click_safe(
        self,
        page: Page,
        target: dict[str, Any],
        *,
        ...
        live_context = page.evaluate(r'''({actionRect}) => {
          ...
          const sourceUrls = Array.from((row || action).querySelectorAll?.('a[href]') || [])
        ...
        live_source_urls = live_context.get('source_urls')
        live_source_urls = live_source_urls if isinstance(live_source_urls, list) else []
        authorized_source_urls = [
            str(value).strip()
            for value in target_source_urls or []
    ...
    def _find_data_acquisition_claim_target(
        ...
        if str(product_query or '').strip():
            ...
            if (
                isinstance(product_target, dict)
                and product_target.get('ok')
        ...
        return page.evaluate(r'''({productQuery, categoryName, storeName, targetSourceUrls}) => {
          ...
          const hasTargetSource = (row) => {
            ...
            return urls.some(url => targetUrls.some(target => url === target));
          };
          const rows = Array.from(document.querySelectorAll(
    ...
    def _find_data_acquisition_claim_target_by_product_query_script(
        ...
        *,
        ...
        script = r'''({productQuery, targetSourceUrls}) => {
          ...
          const targetUrls = Array.isArray(targetSourceUrls) ? targetSourceUrls.filter(Boolean).map(String) : [];
          const sourceMatched = (urls) => urls.some(url => targetUrls.some(target => url === target));
          const query = String(productQuery || '').trim();
          ...
          for (const [index, row] of rows.entries()) {
            ...
            const urls = sourceUrls(row);
            const hasSourceMismatch = targetUrls.length && !sourceMatched(urls);
        ...
        if (
            ...
            and result.get('ok')
            and target_source_urls
            and not self._claim_candidate_has_authorized_source(result, target_source_urls)
        ):
            return None
        if isinstance(result, dict):
            return result
    ...
    def _find_data_acquisition_claim_target_by_source_url(
        ...
        try:
            ...
            count = min(self._locator_count(anchors), 40)
        except Exception:
            anchors = None
            count = 0
        scanned_links = count
        for index in range(count):
            ...
            if not source_url or not self._source_urls_match([source_url], target_source_urls):
                continue
    ...
    def _find_data_acquisition_claim_target_by_product_query_locator(
        ...
        *,
        ...
        for index in range(count):
            ...
            if target_source_urls and (
                not source_urls or not self._source_urls_match(source_urls, target_source_urls)
            ):
                if len(samples) < 6:
                    samples.append({
                        'index': index,
    ...
    def _structured_store_cell_evidence_in_container(
        self,
        container: Any,
        store_name: str | None,
    ) -> dict[str, Any] | None:
        normalized_store = ' '.join(str(store_name or '').split())
        if not normalized_store:
            return None
        try:
            cells = container.locator(
                'td, [role="cell"], .vxe-body--column, .ant-table-cell, '
                '.el-table__cell, [class*="cell"]'
            )
        except Exception:
            return None
        matches: list[tuple[int, dict[str, Any]]] = []
        for index in range(min(self._locator_count(cells), 40)):
            cell = cells.nth(index)
            cell_text = ' '.join(self._locator_text(cell, timeout=500).split())
            compact = ''.join(cell_text.split()).casefold()
            compact_store = ''.join(normalized_store.split()).casefold()
            if compact not in {compact_store, f'「{compact_store}」'} and f'「{compact_store}」' not in compact:
                continue
            matches.append((
                len(cell_text),
                {
                    ...
                    'cell_text': cell_text,
                    'source': 'structured_store_cell',
                    'tag': self._locator_attribute(cell, 'tagName', timeout=300),
                    'class_name': self._locator_attribute(cell, 'class', timeout=300)[:240],
                },
            ))
        if not matches:
            return None
    ...
    def _canonical_source_url_set(self, urls: list[str]) -> set[str]:
        ...
        try:
            for raw_url in urls:
                ...
                if text:
                    canonical.add(canonical_source_identity(text)['primary_url'])
    ...
    def _complete_data_acquisition_claim_dialog(
        ...
        if result.get('skipped'):
            if str(store_name or '').strip():
                return {
                    'ok': False,
                    'reason': '认领确认弹窗未出现，无法证明已选择请求店铺',
                    'requested_store_name': str(store_name).strip(),
                    'dialog_state': result,
                }
            return result
        requested_store_name = ' '.join(str(store_name or '').split())
        option_rects = [item for item in result.get('option_rects') or [] if isinstance(item, dict)]
        if requested_store_name:
            store_option = next(
                (
                    item
                    for item in option_rects
                    if ' '.join(str(item.get('label') or '').split()).casefold()
                    == requested_store_name.casefold()
                ),
                None,
            )
            if store_option is None or not isinstance(store_option.get('rect'), dict) or not self._rect_has_clickable_area(store_option['rect']):
                return {
                    'ok': False,
                    'submitted': False,
                    'reason': f'认领弹窗未找到请求店铺选项 {requested_store_name}，已停止确认',
                    'requested_store_name': requested_store_name,
                    'dialog_state': result,
                }
        clicked_options: list[str] = []
        option_click_receipts: list[dict[str, Any]] = []
        for option in option_rects:
            if not isinstance(option, dict) or not isinstance(option.get('rect'), dict):
                continue
            if not self._rect_has_clickable_area(option['rect']):
                continue
            click_receipt = self._click_data_acquisition_claim_rect_center(page, option['rect'], purpose='认领弹窗选项')
            if not isinstance(click_receipt, dict) or click_receipt.get('dispatched') is not True:
                return {
                    'ok': False,
                    'submitted': False,
                    'reason': f"认领弹窗选项 {str(option.get('label') or option.get('text') or '')} 没有真实点击回执，已停止确认",
                    'requested_store_name': requested_store_name or None,
                    'clicked_options': clicked_options,
                    'dialog_state': result,
                }
            clicked_options.append(str(option.get('label') or option.get('text') or ''))
            option_click_receipts.append(dict(click_receipt))
            try:
        ...
        if requested_store_name:
            readback_script = r'''() => {
              ...
              const containers = Array.from(document.querySelectorAll('.ant-modal, .ant-modal-wrap, .el-dialog, [role="dialog"], .modal, .layui-layer'))
                .filter(visible)
                .filter(el => textOf(el).includes('认领') || textOf(el).includes('领取') || textOf(el).includes('采集箱'));
              const dialog = containers[containers.length - 1];
              if (!dialog) return {ok:false, requested_store_name:storeName, observed_store_name:null, selected:false, reason:'认领弹窗已消失，无法回读店铺选择'};
              const optionSelector = 'label,.ant-checkbox-wrapper,.el-checkbox,.ant-radio-wrapper,.el-radio-wrapper,li,[role="treeitem"],[role="option"]';
              const exactTextNode = Array.from(dialog.querySelectorAll(`${optionSelector},button,a,span,div`))
                .filter(visible)
                .find(el => norm(textOf(el)) === norm(storeName));
              const exact = exactTextNode && (exactTextNode.closest(optionSelector) || exactTextNode);
              if (!exact) return {ok:false, requested_store_name:storeName, observed_store_name:null, selected:false, reason:'无法回读请求店铺选项'};
              const input = exact.matches('input') ? exact : exact.querySelector('input[type="checkbox"],input[type="radio"]');
              const ariaChecked = exact.getAttribute('aria-checked') || (input && input.getAttribute('aria-checked')) || null;
              const ariaSelected = exact.getAttribute('aria-selected') || null;
              const className = String(exact.className || '');
              const inputChecked = Boolean(input && input.checked);
              const selected = inputChecked || ariaChecked === 'true' || ariaSelected === 'true' || /(?:^|[-_\s])(checked|selected)(?:$|[-_\s])/i.test(className);
              const selectedOptionNames = [];
              const seenOptions = new Set();
              const optionRoots = Array.from(dialog.querySelectorAll(optionSelector));
              for (const option of optionRoots) {
                ...
                const optionClass = String(option.className || '');
                const optionSelected = Boolean(
                  (optionInput && optionInput.checked)
                  || option.getAttribute('aria-checked') === 'true'
                  || option.getAttribute('aria-selected') === 'true'
                  || (optionInput && optionInput.getAttribute('aria-checked') === 'true')
                  || /(?:^|[-_\s])(checked|selected)(?:$|[-_\s])/i.test(optionClass)
                );
                if (!optionSelected) continue;
                const optionName = textOf(option);
                const optionKey = norm(optionName);
                if (!optionKey || seenOptions.has(optionKey)) continue;
                seenOptions.add(optionKey);
                selectedOptionNames.push(optionName);
              }
              const selectedStoreNames = selectedOptionNames.filter(name => norm(name) !== norm(categoryName));
              return {
                ok:true,
                requested_store_name:storeName,
                observed_store_name:textOf(exact),
                selected,
                selected_store_names:selectedStoreNames,
                selection_evidence:{
                  input_checked:inputChecked,
                  aria_checked:ariaChecked,
                  aria_selected:ariaSelected,
                  class_name:className,
                  selected_option_names:selectedOptionNames,
                },
              };
            }'''.replace('__STORE_NAME__', json.dumps(requested_store_name, ensure_ascii=False)).replace('__CATEGORY_NAME__', category_json)
            try:
                readback = self._evaluate_zero_arg_page_function_with_runtime_timeout(page, readback_script, timeout=1500)
            except Exception as exc:  # noqa: BLE001 - a missing readback is a fail-closed proof failure.
                readback = {'ok': False, 'selected': False, 'reason': f'店铺选择回读失败：{str(exc)[:180]}'}
            store_selection = dict(readback) if isinstance(readback, dict) else {'ok': False, 'selected': False}
            observed_store_name = ' '.join(str(store_selection.get('observed_store_name') or '').split())
            selected_store_names: list[str] = []
            selected_store_keys: set[str] = set()
            for raw_name in store_selection.get('selected_store_names') or []:
                selected_name = ' '.join(str(raw_name or '').split())
                selected_key = selected_name.casefold()
                if selected_name and selected_key not in selected_store_keys:
                    selected_store_keys.add(selected_key)
                    selected_store_names.append(selected_name)
            store_selection['selected_store_names'] = selected_store_names
            if (
                store_selection.get('ok') is not True
                or store_selection.get('selected') is not True
                or observed_store_name.casefold() != requested_store_name.casefold()
                or not isinstance(store_selection.get('selection_evidence'), dict)
                or not store_selection.get('selection_evidence')
                or len(selected_store_names) != 1
                or selected_store_names[0].casefold() != requested_store_name.casefold()
            ):
                return {
                    'ok': False,
                    'submitted': False,
                    'reason': store_selection.get('reason') or '无法证明仅请求店铺被真实选中，已停止确认',
                    'requested_store_name': requested_store_name,
        ...
        submit_click_receipt = self._click_data_acquisition_claim_rect_center(
            page,
            submit_rect,
            purpose='认领弹窗确认',
            mutation_action='claim_confirm_click',
            pre_dispatch_guard=lambda: self._assert_data_acquisition_claim_dialog_submit_safe(
                page,
        ...
        updated['option_click_receipts'] = option_click_receipts
        updated['submit_click_receipt'] = submit_click_receipt
        if store_selection is not None:
            updated['store_selection'] = store_selection
        return updated

    def _assert_data_acquisition_claim_dialog_submit_safe(
        self,
        page: Page,
        *,
        submit_rect: dict[str, Any],
        requested_store_name: str,
        category_name: str | None,
    ) -> dict[str, Any]:
        """Re-read the selected store and exact confirm button at click time."""

        payload = {
            'storeName': str(requested_store_name or '').strip(),
            'categoryName': str(category_name or '').strip(),
            'expectedRect': dict(submit_rect),
        }
        result = self._evaluate_zero_arg_page_function_with_runtime_timeout(
            page,
            r'''() => {
              ...
              const forbidden = ['发布','立即发布','继续发布','保存并发布','保存并移入待发布','移入待发布','批量发布'];
              const dialogs = Array.from(document.querySelectorAll('.ant-modal,.ant-modal-wrap,.el-dialog,[role="dialog"],.modal,.layui-layer'))
                .filter(visible)
                .filter(el => /认领|领取|采集箱/.test(textOf(el)));
              const dialog = dialogs[dialogs.length - 1];
              if (!dialog) return {ok:false, reason:'claim_dialog_missing'};
              if (forbidden.some(term => norm(textOf(dialog)).includes(norm(term)))) {
                return {ok:false, reason:'claim_dialog_contains_publish_action'};
              }
              const buttons = Array.from(dialog.querySelectorAll('button,a,[role="button"]'))
                .filter(visible)
                .filter(el => !el.disabled && el.getAttribute('aria-disabled') !== 'true');
              const submitCandidates = buttons.filter(el => {
                const text = norm(textOf(el));
                return ['确定','确认','提交','开始认领','认领','领取'].includes(text);
              });
              if (submitCandidates.length !== 1) {
                return {ok:false, reason:'claim_submit_not_unique', count:submitCandidates.length};
              }
              const actualRect = rectOf(submitCandidates[0]);
              const expectedRect = payload.expectedRect || {};
              const rectStable = ['x','y','w','h'].every(key =>
                Math.abs(Number(actualRect[key] || 0) - Number(expectedRect[key] || 0)) <= 2
              );
              if (!rectStable) return {ok:false, reason:'claim_submit_rect_changed', actual_rect:actualRect};
              const optionSelector = 'label,.ant-checkbox-wrapper,.el-checkbox,.ant-radio-wrapper,.el-radio-wrapper,li,[role="treeitem"],[role="option"]';
              const selectedNames = Array.from(dialog.querySelectorAll(optionSelector))
                .filter(visible)
                .filter(option => {
                  ...
                  return Boolean(
                    ...
                    || option.getAttribute('aria-checked') === 'true'
                    || option.getAttribute('aria-selected') === 'true'
                    || /(?:^|[-_\s])(checked|selected)(?:$|[-_\s])/i.test(cls)
                  );
                })
                .map(textOf)
                .filter(name => norm(name) && norm(name) !== norm(payload.categoryName));
              const uniqueStores = Array.from(new Set(selectedNames.map(name => norm(name))));
              if (uniqueStores.length !== 1 || uniqueStores[0] !== norm(payload.storeName)) {
                return {ok:false, reason:'claim_store_selection_changed', selected_stores:selectedNames};
              }
              return {ok:true, submit_text:textOf(submitCandidates[0]), submit_rect:actualRect, selected_stores:selectedNames};
            }'''.replace('__PAYLOAD__', json.dumps(payload, ensure_ascii=False)),
            timeout=1500,
        )
        if not isinstance(result, dict) or result.get('ok') is not True:
            reason = (
                str(result.get('reason') or 'claim dialog target changed')
                if isinstance(result, dict)
                else 'claim dialog target probe unavailable'
            )
    ...
    def _verify_draft_box_claim(
        ...
        claim_dialog = state.get('claim_dialog') if isinstance(state.get('claim_dialog'), dict) else {}
        store_selection = (
            claim_dialog.get('store_selection')
            if isinstance(claim_dialog.get('store_selection'), dict)
            else None
        )
        ...
        if not native_navigated and visible_browser:
            ...
            try:
                ...
                if not (visible_from_data_acquisition and self._is_current_page_url(page, draft_box_url)):
                    raise
                self._trace_workflow_event(
                    'draft_box_claim_verify:sterile_goto_timeout_continue',
                    error=str(exc)[:240],
                    current_url=getattr(page, 'url', None),
                    human_step='商品箱页面已开始切换，继续等待页面稳定',
                )
            self._trace_workflow_event(
        ...
        if self._page_looks_like_login(page):
            ...
            try:
                ...
                page_title = None
            self._trace_workflow_event(
                'draft_box_claim_verify:login_required',
                current_url=getattr(page, 'url', None),
                page_title=page_title,
                human_step='商品箱页面要求重新登录',
            )
            return {
                'ok': False,
        ...
        if visible_draft_box:
            ...
            self._trace_workflow_event(
                'draft_box_claim_verify:readiness_gate',
                readiness=readiness,
                human_step='检查商品箱页面是否可操作',
            )
            if readiness.get('ok') is not True:
                return {
                    'ok': False,
                    'stage': 'draft_box_claim_page_not_ready',
                    'label': '商品箱页面未就绪',
                    'message': readiness.get('message') or '商品箱页面尚未达到可操作状态。',
                    'next_action': '请在真实浏览器中处理登录、加载或弹窗后重新执行商品箱确认。',
                    'requires_user_action': True,
                    'reason': readiness.get('reason'),
                    'readiness': readiness,
                    'page_title': readiness.get('page_title'),
                    'page_url': readiness.get('page_url') or getattr(page, 'url', None),
                    'screenshot_url': None,
                    'browser_visible': not self._is_headless(),
                    'updated_at': now_iso(),
                }
            wait_result = {
                'ready': True,
                'ready_term': 'browser_readiness_gate',
                'loading': readiness.get('loading'),
                'rows': None,
                'text_excerpt': readiness.get('body_excerpt') or '',
                'url': readiness.get('page_url') or getattr(page, 'url', ''),
                'title': readiness.get('page_title') or '',
            }
        else:
            wait_result = self._wait_for_page_ready(
                page,
        ...
        screenshot_path = self._scoped_evidence_path(
            ACQUISITION_ACTION_SCREENSHOT_MAP['verify'],
        ...
        return {
            ...
            'target_row_text': row_info.get('rowText'),
            'target_source_urls': row_info.get('sourceUrls', []),
            'draft_box_match': draft_box_match,
            'claimed_product': {
    ...
    def _semantic_draft_box_match(
        ...
        *,
        ...
        observed_source_urls = [
            str(value).strip()
            for value in row_info.get('sourceUrls') or []
            if str(value or '').strip()
        ]
        if not observed_source_urls:
            raise RuntimeError('商品箱真实行缺少可审计的来源链接')
        raw_matched_by = str(row_info.get('matchedBy') or '').strip()
        canonical_observed_urls: dict[str, str] = {}
        canonical_target_urls: set[str] = set()
        try:
            for observed in observed_source_urls:
                canonical = canonical_source_identity(observed)['primary_url']
                canonical_observed_urls.setdefault(canonical, observed)
            for target in target_source_urls:
        ...
        if not isinstance(store_cell_evidence, dict):
            raise RuntimeError('商品箱真实行缺少结构化店铺单元格证据')
        cell_store_name = ' '.join(str(store_cell_evidence.get('store_name') or '').split())
        cell_text = ' '.join(str(store_cell_evidence.get('cell_text') or '').split())
        if (
            not normalized_store
            or store_cell_evidence.get('source') != 'structured_store_cell'
            or cell_store_name.casefold() != normalized_store
            or not self._contains_exact_observed_hint(cell_text.casefold(), normalized_store)
        ):
            raise RuntimeError('商品箱结构化店铺单元格没有实际命中授权店铺')
        store_selection = dict(store_selection or {})
        observed_store_name = ' '.join(str(store_selection.get('observed_store_name') or '').split())
        selection_evidence = store_selection.get('selection_evidence')
        ...
        if (
            store_selection.get('selected') is not True
            or observed_store_name.casefold() != normalized_store
            or len(selected_store_names) != 1
            or selected_store_names[0].casefold() != normalized_store
            or not isinstance(selection_evidence, dict)
            or not selection_evidence
        ):
            raise RuntimeError('认领弹窗没有可审计的真实店铺选择回读')
        return {
            'matched_by': matched_by,
            'matched_value': matched_value,
            'raw_matched_by': raw_matched_by,
            'row_text': row_text,
            'source_urls': observed_source_urls,
            'store_name': observed_store_name,
            'store_evidence': dict(store_cell_evidence),
            'store_observation': {
    ...
    def _contains_exact_observed_hint(self, normalized_text: str, normalized_hint: str) -> bool:
        if not normalized_text or not normalized_hint:
            return False
        if re.search(r'[a-z0-9]', normalized_hint):
    ...
    def _perform_editor_action(
        ...
        if action in {
            ...
            if not self._is_same_dxm_editor_page(getattr(page, 'url', ''), editor_url):
                ...
                if open_editor_page is not None:
                    page = open_editor_page
                    self._page = page
                    self._trace_workflow_event(
                        'editor_action:reuse_open_editor_page',
                        action=action,
                        current_url=getattr(page, 'url', None),
                        human_step='复用已打开编辑页',
                    )
            ...
            ready = (
                self._wait_for_visible_editor_loaded(page, expected_identity='editor', product_query=product_query, timeout=20000)
                if visible_editor_page
                else self._wait_for_body_text(page, ['基本信息', '半托管服务', '产品信息'], expected_identity='editor')
            )
            self._trace_workflow_event(
            ...
            if not ready and not has_known_editor_url:
                draft_url = WORKFLOW_TARGETS['draft_box']['url']
                self._goto_with_live_hud(page, draft_url, wait_until='domcontentloaded', timeout=45000)
                self._wait_for_page_ready(
                    page,
                ...
                self._dismiss_blocking_modals(page)
                frozen_row = self._find_draft_box_row(
                    page,
                    store_name=store_name,
                    target_source_urls=target_source_urls,
                    target_identity=normalized_target,
                )
                page = self._open_frozen_target_editor_from_draft_box(
                    page,
                    row_info=frozen_row,
                    target_identity=normalized_target,
                    store_name=store_name,
                ...
                self._page = page
                reopened_ready = self._wait_for_body_text(
                    page,
                    ['基本信息', '半托管服务', '产品信息'],
        ...
        if action in {'save_only', 'verify_not_published'} and self._is_dxm_editor_url(state_page_url):
            ...
            if not self._is_same_dxm_editor_page(getattr(page, 'url', ''), editor_url):
                ...
                if open_editor_page is not None:
                    ...
                    self._page = page
                    self._trace_workflow_event(
                        'editor_action:sterile_reuse_open_editor_page',
                        action=action,
                        current_url=getattr(page, 'url', None),
                        human_step='复用已打开编辑页',
                    )
    ...
    def _find_open_editor_page_for_url(self, editor_url: str | None) -> Page | None:
        if not self._is_dxm_editor_url(editor_url):
            return None
        for candidate in self._context_pages():
            ...
            try:
                ...
                continue
            if self._is_same_dxm_editor_page(candidate_url, editor_url):
                return candidate
        return None

    def _find_open_editor_page_for_target(
        self,
        *,
        product_query: str | None,
        target_source_urls: list[str] | None,
        ...
        for candidate in self._context_pages():
            ...
            try:
                ...
                continue
            if not self._is_dxm_editor_url(candidate_url):
                continue
            match = self._editor_page_matches_target(
                candidate,
                product_query=product_query,
                target_source_urls=target_source_urls or [],
                target_identity=target_identity,
    ...
    def _editor_page_matches_target(
        ...
        *,
        ...
        try:
            result = page.evaluate(r'''({frag, targetSourceUrls}) => {
              ...
              const values = [bodyText,
                ...Array.from(document.querySelectorAll('input,textarea')).map(el => norm(el.value || el.getAttribute('value') || '')),
                ...Array.from(document.querySelectorAll('a[href]')).map(el => String(el.href || el.getAttribute('href') || '')),
              ].filter(Boolean);
              const sourceUrls = Array.from(new Set([
                ...Array.from(document.querySelectorAll('input,textarea')).map(el => canonicalUrl(el.value || el.getAttribute('value') || '')),
                ...Array.from(document.querySelectorAll('a[href]')).map(el => canonicalUrl(el.href || el.getAttribute('href') || '')),
              ].filter(Boolean)));
              const matchedSourceUrls = sourceUrls.filter(url => targets.includes(url));
              const query = norm(frag);
              const queryPrefix = query.length >= 18 ? query.slice(0, 18) : query;
              const titleMatched = Boolean(query && values.some(value => value.includes(query) || (queryPrefix.length >= 12 && value.includes(queryPrefix))));
              if (matchedSourceUrls.length) {
                return {
                  ok: true,
                  matchedBy: 'source_url',
                  sourceUrls: matchedSourceUrls.slice(0, 5),
                  titleMatched,
    ...
    def _wait_for_visible_editor_loaded(
        ...
        *,
        ...
        while True:
            ...
            if identity_result.get('ok') is not True:
                self._trace_workflow_event(
                    'visible_editor_ready_wrong_route',
                    expected_identity=expected_identity,
                    identity_result=identity_result,
                    current_url=getattr(page, 'url', None),
                    human_step='编辑页路由校验失败',
                )
                return False
    ...
    def _visible_editor_ready_state(self, page: Page, *, product_query: str | None = None) -> dict[str, Any]:
        script = r'''({productQuery}) => {
          ...
          const markerNodes = Array.from(document.querySelectorAll(
            'h1,h2,h3,label,button,[role="button"],[role="tab"],.ant-tabs-tab,.el-tabs__item,[class*="form-item"],[class*="FormItem"]'
          )).filter(visible);
          const markerTexts = markerNodes
            .map(el => String(el.innerText || el.textContent || el.getAttribute('aria-label') || '').replace(/\s+/g, ' ').trim())
            .filter(Boolean);
          const markerCompact = markerTexts.map(norm);
          const query = String(productQuery || '').trim();
          ...
          const structuredValues = values.map(norm).concat(markerCompact);
          const storeMissing = structuredValues.some(value => value.includes('----请选择店铺----') || value.includes('请选择店铺'));
          ...
          return {
            ...
            business_markers: markerTexts.slice(0, 12),
    ...
    def _fill_editor_required_defaults_on_page(
        ...
        *,
        ...
        blocking_reference_missing = list(reference_missing)
        ...
        if self._is_visible_dxm_editor_page(page):
            category_editor_state = category.get('editor_state') if isinstance(category, dict) else None
            category_editor_values = (
                category_editor_state.get('values')
                if isinstance(category_editor_state, dict) and isinstance(category_editor_state.get('values'), dict)
                else {}
            )
            existing_state_title = str(category_editor_values.get('title') or '').strip()
            title_action = self._resolve_template_title_action(
                values,
                configured_values,
            ...
            if title_action.get('ok') and not title_action.get('write'):
                ...
                self._trace_workflow_event(
                    'editor_base_fields:title_preserved_from_state',
                    value=existing_state_title[:120],
                    current_url=getattr(page, 'url', None),
                    human_step='产品标题已有值，跳过重复填写',
                )
            elif title_action.get('ok'):
                next_title = str(title_action.get('value') or '').strip()
                title_fill = self._fill_visible_editor_title_with_native_input(page, next_title, force_replace=True)
                if not title_fill.get('ok'):
                    title_fill = self._fill_text_inputs_near_label(page, '产品标题', [next_title])
                title_ok = bool(title_fill.get('ok'))
                title_strategy = str(title_action.get('strategy') or 'template_title')
                self._trace_workflow_event(
                    'editor_base_fields:title_template_applied',
                    before=existing_state_title[:120],
                    after=next_title[:120],
                    fill_ok=title_ok,
                    strategy=title_strategy,
                    current_url=getattr(page, 'url', None),
                    human_step='按模板策略处理产品标题',
                )
            else:
                return {
                    'ok': False,
                    'stage': 'fill_editor_required_defaults_failed',
                    'label': '标题模板未就绪',
                    'message': str(title_action.get('reason') or '产品标题含中文或为空，且没有可执行的标题模板策略。'),
                    'page_title': '店小秘编辑页',
                    'page_url': page.url,
                    'screenshot_url': None,
                    'fill_result': {
                        ...
                        'fields': {
                            **field_result,
                            'title': False,
                            'title_strategy': str(title_action.get('strategy') or ''),
                            'title_value': existing_state_title,
                        },
            ...
            goods_code_action = self._resolve_template_goods_code_action(
                values,
                configured_values,
            ...
            if goods_code_action.get('ok') and not goods_code_action.get('write'):
                sku_ok = True
                sku_strategy = str(goods_code_action.get('strategy') or 'preserve_existing_visible_editor_value')
            elif goods_code_action.get('ok'):
                sku_fill = self._fill_text_inputs_near_label(page, '商品编码', [safe_sku_code])
                sku_ok = bool(sku_fill.get('ok'))
                sku_strategy = str(goods_code_action.get('strategy') or 'template_goods_code')
                self._trace_workflow_event(
                    'editor_base_fields:goods_code_template_applied',
                    before=existing_sku_code[:80],
                    after=safe_sku_code,
                    fill_ok=sku_ok,
                    current_url=getattr(page, 'url', None),
                    human_step='按模板策略处理商品编码',
                )
            else:
                return {
                    'ok': False,
                    'stage': 'fill_editor_required_defaults_failed',
                    'label': '商品编码模板未就绪',
                    'message': str(goods_code_action.get('reason') or '商品编码不符合店小秘规则，且没有可执行的 SKU/货号模板策略。'),
                    'page_title': '店小秘编辑页',
                    'page_url': page.url,
                    'screenshot_url': None,
                    'fill_result': {
                        ...
                        'fields': {
                            **field_result,
                            'sku_code': False,
                            'sku_code_strategy': str(goods_code_action.get('strategy') or ''),
                            'sku_code_value': existing_sku_code,
                        },
        ...
        reference_templates = dxm_reference_template_results.get('attribute_info')
        if reference_templates and reference_templates.get('ok') and not reference_templates.get('skipped'):
            category_attributes = {
                'ok': True,
                'skipped': True,
                'via_template': True,
            ...
            self._trace_workflow_event(
                'category_attributes:covered_by_reference_template',
                section='attribute_info',
                names=list(reference_templates.get('names') or [])[:5],
                current_url=getattr(page, 'url', None),
                human_step='属性引用模板已套用，跳过手动属性填写',
            )
        else:
            category_attributes = {
                'ok': False,
            ...
            self._trace_workflow_event(
                'category_attributes:manual_fill_skipped',
                section='attribute_info',
                current_url=getattr(page, 'url', None),
        ...
        def missing_field_is_satisfied(item: str) -> bool:
            if item == 'english_title':
                return bool(field_result.get('title')) and self._is_safe_english_title(validated_title)
            if item == 'delivery_days':
                return bool(field_result.get('delivery_days'))
            return False
    ...
    def _fill_visible_editor_title_with_native_input(self, page: Page, title: str, *, force_replace: bool = False) -> dict[str, Any]:
        existing_title = self._visible_editor_existing_title_value(page)
        if existing_title and not force_replace:
            self._trace_workflow_event(
                'visible_editor_title:existing_value',
                value=existing_title[:120],
                current_url=getattr(page, 'url', None),
                human_step='产品标题已存在，跳过重复填写',
            )
            return {
                'ok': True,
                'already_present': True,
                'method': 'dom_existing_value',
            ...
            try:
                ...
                if self._replace_active_field_with_native_clipboard_text(title):
                    ...
                    if (
                        ...
                        self._trace_workflow_event(
                            'visible_editor_title:native_value_mismatch',
                            point=point,
                            expected=title[:120],
                            actual=confirmed_title[:120],
                            current_url=getattr(page, 'url', None),
                            human_step='产品标题写入未生效',
                        )
                        continue
                    self._trace_workflow_event(
    ...
    def _visible_editor_existing_title_value(self, page: Page) -> str:
        selectors = self._prefer_visible_css_selectors(
            self._known_visible_editor_text_selectors('产品标题')
        )
        for selector in selectors:
            try:
            ...
            for index in range(count):
                try:
                    candidate = locator.nth(index).input_value(timeout=600).strip()
                except Exception:
                    continue
                if candidate:
                    return candidate
    ...
    def _fill_editor_variants_on_page(
        ...
        *,
        ...
        if result.get('variant_scope_found') and int(result.get('logistics_icon_count') or 0) > 0:
            ...
            result['logistics_attribute_detail'] = {
                'ok': False,
                'write_attempted': False,
    ...
    def _verify_editor_variant_logistics_attribute(self, page: Page, value: str) -> dict[str, Any]:
        try:
            result = page.evaluate(r'''(value) => {
              ...
              if (!scope) return {ok:false, located:false, expected_value:value, value_after:'', row_count:0, filled_count:0, missing_rows:[], reason:'未找到普通变体表格'};
              ...
              const readings = rows.map((row, index) => {
                const nodes = Array.from(row.querySelectorAll([
                  '[data-logistics-attribute]',
                  '[data-field="logistics_attribute"]',
                  'input[name*="logistics" i]',
                  'select[name*="logistics" i]'
                ].join(','))).filter(visible);
                const observedValues = nodes.flatMap(node => [
                  node.getAttribute('data-logistics-attribute'),
                  node.getAttribute('data-value'),
                  node.value,
                  node.tagName === 'SELECT' ? node.selectedOptions?.[0]?.textContent : null,
                  textOf(node),
                ]).map(item => String(item || '').trim()).filter(Boolean);
                return {
                  index,
                  located: nodes.length > 0,
                  observed_values: Array.from(new Set(observedValues)),
                  exact: observedValues.some(item => norm(item) === norm(value)),
                };
              });
              const missingRows = readings.filter(row => !row.located || !row.exact);
              const exact = rows.length > 0 && missingRows.length === 0;
              return {
                ok: exact,
                located: rows.length > 0 && readings.every(row => row.located),
                expected_value: String(value || '').trim(),
                value_after: exact ? String(value || '').trim() : '',
    ...
    def _flatten_editor_defaults(self, defaults: dict[str, Any] | None) -> dict[str, Any]:
        ...
        groups = {
            'category': {
                ...
                'title_strategy': ('title_strategy',),
                'title_cleaning_rule': ('title_cleaning_rule',),
                'title_keyword_map': ('title_keyword_map',),
                'english_title_fallback': ('english_title_fallback', 'title_fallback'),
            ...
            'sku': {
                ...
                'goods_code_strategy': ('goods_code_strategy', 'template_sku_rule'),
            ...
            'semi_managed': {
                ...
                'is_original_box': ('is_original_box', 'original_box'),
                'logistics_attribute': ('logistics_attribute', 'attribute'),
                'goods_code_strategy': ('goods_code_strategy',),
            },
        }
    ...
    @staticmethod
    def _is_safe_dxm_goods_code(value: Any) -> bool:
        text = str(value or '').strip()
        return bool(text) and len(text) <= 50 and re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*', text) is not None

    @staticmethod
    def _has_chinese_text(value: Any) -> bool:
        return re.search(r'[\u3400-\u9fff]', str(value or '')) is not None

    def _normalize_template_english_title(self, value: Any, *, max_length: int = 120) -> str:
    ...
    def _is_safe_english_title(self, value: Any) -> bool:
        text = str(value or '').strip()
        return bool(text) and not self._has_chinese_text(text) and re.search(r'[A-Za-z]', text) is not None

    def _resolve_template_title_action(
        self,
        values: dict[str, Any],
        ...
        *,
        ...
        if self._is_safe_english_title(existing):
            return {
                'ok': True,
                'write': False,
                'value': existing,
                'strategy': 'preserve_existing_visible_editor_state',
            }

        for key in ('title_override', 'english_title', 'title'):
            candidate = str(configured_values.get(key) or '').strip()
            if candidate and self._is_safe_english_title(candidate):
                return {
                    'ok': True,
                    'write': True,
                    'value': self._normalize_template_english_title(candidate),
                    'strategy': f'template_{key}',
                }

        strategy = str(configured_values.get('title_strategy') or values.get('title_strategy') or '').strip()
        if not strategy:
            return {
                'ok': False,
                'strategy': '',
                'missing': ['category.title_strategy'],
                'reason': '产品标题含中文或为空；请先在“类目与标题”模板配置英文标题策略，系统不会默认手工改写标题。',
            }

        normalized = strategy.lower().replace(' ', '').replace('_', '').replace('-', '')
        source_terms = (
            '来源标题',
            '商品标题',
            '原始标题',
        ...
        if any(term.lower().replace(' ', '').replace('_', '').replace('-', '') in normalized for term in source_terms):
            source_title = self._source_title_from_values(values, configured_values)
            generated = self._template_english_title_from_source(source_title, values, configured_values)
            if self._is_safe_english_title(generated):
                return {
                    'ok': True,
                    'write': True,
                    'value': generated,
                    'strategy': 'template_source_title_english',
                }
            fallback = str(configured_values.get('english_title_fallback') or values.get('english_title_fallback') or '').strip()
            if self._is_safe_english_title(fallback):
                return {
                    'ok': True,
                    'write': True,
                    'value': self._normalize_template_english_title(fallback),
                    'strategy': 'template_english_title_fallback',
                }
            return {
                'ok': False,
                'strategy': 'template_source_title_english',
                'missing': ['source_title'],
                'reason': '标题模板要求按来源标题生成英文标题，但当前来源标题无法生成合规英文标题；请在模板中配置英文标题覆盖值或关键词映射。',
            }

        preserve_terms = ('保留', '沿用', 'preserve', 'keep')
        if any(term in normalized for term in preserve_terms):
            return {
                'ok': False,
                'strategy': 'preserve_source_title',
                'missing': ['category.title_strategy'],
                'reason': f'当前标题策略为“{strategy}”，但现有产品标题含中文，速卖通编辑页不允许保存；请改为“按来源标题生成英文标题”或填写英文标题覆盖值。',
            }

        return {
            'ok': False,
            'strategy': 'unsupported_title_strategy',
            'missing': ['category.title_strategy'],
            'reason': f'当前标题策略“{strategy}”尚未支持执行；请改为“按来源标题生成英文标题”或填写英文标题覆盖值。',
        }

    def _source_title_from_values(self, values: dict[str, Any], configured_values: dict[str, Any]) -> str:
        for source in (configured_values, values):
            for key in ('source_title', 'original_title', 'product_title', 'keyword'):
                text = str(source.get(key) or '').strip()
                if text:
                    return text
        return ''

    def _template_english_title_from_source(
        self,
        source_title: str,
        values: dict[str, Any],
        configured_values: dict[str, Any],
        ...
        if self._is_safe_english_title(source):
            return self._normalize_template_english_title(source)

        configured_map = configured_values.get('title_keyword_map') or values.get('title_keyword_map')
        ...
        for zh, en in keyword_map:
            zh_text = zh.strip()
            en_text = self._normalize_template_english_title(en)
            if not zh_text or not en_text or zh_text not in source:
                continue
            key = en_text.lower()
            if key in seen:
                continue
        ...
        if not parts:
            category_match = self._normalize_template_english_title(
                configured_values.get('category_match') or values.get('category_match') or ''
            )
            if category_match and self._is_safe_english_title(category_match):
                parts.append(category_match)
                parts.append('Collectible Product')

        return self._normalize_template_english_title(' '.join(parts))

    def _safe_dxm_goods_code(self, value: Any, *, fallback: Any = '') -> str:
        ...
        for candidate in (value, fallback):
            safe = normalize(candidate)
            if self._is_safe_dxm_goods_code(safe):
                return safe
        seed = str(value or fallback or 'dxm-goods-code').encode('utf-8', errors='ignore')
        return 'DXM-' + hashlib.sha1(seed).hexdigest()[:12].upper()

    def _resolve_template_goods_code_action(
        self,
        values: dict[str, Any],
        configured_values: dict[str, Any],
        *,
        ...
        existing = str(existing_sku_code or '').strip()
        if self._is_safe_dxm_goods_code(existing):
            return {
                'ok': True,
                'write': False,
                'value': existing,
                'strategy': 'preserve_existing_visible_editor_value',
            }

        configured_sku_code = str(configured_values.get('sku_code') or '').strip()
        if self._is_safe_dxm_goods_code(configured_sku_code):
            return {
                'ok': True,
                'write': True,
                'value': configured_sku_code,
                'strategy': 'template_sku_code',
            }

        strategy = str(
            configured_values.get('goods_code_strategy')
            or values.get('goods_code_strategy')
            or ''
        ).strip()
        if not strategy:
            return {
                'ok': False,
                'strategy': '',
                'missing': ['sku.goods_code_strategy'],
                'reason': '商品编码不符合店小秘规则；请先在模板中心配置 SKU 编码或货号策略，系统不会默认手工改写。',
            }

        normalized = strategy.lower().replace(' ', '').replace('_', '').replace('-', '')
        preserve_terms = ('沿用店小秘生成', '保持店小秘', 'preserve', 'keep', 'dxm')
        if any(term.lower().replace(' ', '').replace('_', '').replace('-', '') in normalized for term in preserve_terms):
            return {
                'ok': False,
                'strategy': 'preserve_dxm_generated',
                'missing': ['sku.goods_code_strategy'],
                'reason': f'当前模板货号策略为“{strategy}”，但店小秘现有商品编码不合规；请改为“按来源商品ID生成安全货号”或填写明确 SKU 编码。',
            }
        ...
        if any(term in normalized for term in source_terms):
            source_goods_id = self._source_goods_id_from_values(values)
            if self._is_safe_dxm_goods_code(source_goods_id):
                return {
                    'ok': True,
                    'write': True,
                    'value': source_goods_id,
                    'strategy': 'template_source_goods_id',
                }
            return {
                'ok': False,
                'strategy': 'template_source_goods_id',
                'missing': ['source_urls'],
                'reason': '模板要求按来源商品ID生成货号，但当前任务没有可识别的来源商品链接。',
            }

        sanitize_terms = ('清理非法字符', '合规化', 'sanitize', 'clean')
        if any(term in normalized for term in sanitize_terms):
            safe = self._safe_dxm_goods_code(existing, fallback=configured_sku_code)
            return {
                'ok': True,
                'write': True,
                'value': safe,
                'strategy': 'template_sanitize_existing',
            }

        return {
            'ok': False,
            'strategy': 'unsupported_goods_code_strategy',
            'missing': ['sku.goods_code_strategy'],
            'reason': f'当前模板货号策略“{strategy}”尚未支持执行；请改为“按来源商品ID生成安全货号”或填写明确 SKU 编码。',
        }

    def _source_goods_id_from_values(self, values: dict[str, Any]) -> str:
        raw_values: list[Any] = []
        for key in ('source_url', 'url'):
            if values.get(key):
                raw_values.append(values.get(key))
        source_urls = values.get('source_urls')
        if isinstance(source_urls, (list, tuple, set)):
            raw_values.extend(source_urls)
        elif source_urls:
            raw_values.append(source_urls)
        ...
        for raw in raw_values:
            ...
            parsed = urlparse(text)
            query = parse_qs(parsed.query)
            for key in ('goods_id', 'goodsId', 'offer_id', 'offerId', 'id'):
                for candidate in query.get(key, []):
                    if self._is_safe_dxm_goods_code(candidate):
                        return str(candidate)
            path_match = re.search(r'/(?:offer|goods|item)/(\d{6,})(?:\.html)?', parsed.path)
            if path_match:
                return path_match.group(1)
            any_match = re.search(r'(?<!\d)(\d{9,})(?!\d)', text)
            if any_match:
    ...
    def _image_slot_state(self, page: Page, slot_label: str, filename: str) -> dict[str, Any]:
        return page.evaluate(r'''({slotLabel, filename}) => {
          ...
          return {
            ok: Boolean(hasConfiguredName),
            reason: hasConfiguredName ? null : (filledImgs.length ? 'configured_filename_exact_readback_missing' : (slotLabel === '外包装/标签实拍图-欧盟' ? '欧盟外包装图槽位仍为空' : `${slotLabel}槽位仍为空`)),
    ...
    def _select_editor_category(self, page: Page, keyword: str, match_text: str) -> dict[str, Any]:
        ...
        if body_state.get('category_selected'):
            return {
                ...
                'text': body_state.get('category_text'),
                'editor_state': body_state,
            }
    ...
    def _apply_reference_templates_on_page(self, page: Page, priorities: list[str]) -> dict[str, Any]:
        ...
        script = r'''(priorities) => {
          ...
          const containsAny = (text, terms) => terms.some(term => text.includes(term) || norm(text).includes(norm(term)));
          const attributeSectionTerms = ['属性信息', '产品属性', '类目属性'];
          const templateTerms = ['引用模板', '产品属性模板', '属性模板', '请选择【产品属性模板】', '请选择模板'];
          const root = Array.from(document.querySelectorAll('section,form,div'))
            .filter(visible)
            .find(el => {
              const text = textOf(el);
              return containsAny(text, attributeSectionTerms) && containsAny(text, templateTerms);
            });
    ...
    def _visible_editor_text_input_state(self, page: Page, label_text: str) -> dict[str, Any]:
        ...
        for selector in self._prefer_visible_css_selectors(selectors):
            try:
                ...
                if inputs.count() <= 0:
                    continue
                target = inputs.nth(0)
                value = target.input_value(timeout=1200)
                return {'ok': True, 'value': value, 'method': 'visible_known_selector', 'selector': selector}
            except Exception:
                continue
        try:
            state = page.evaluate(r'''(labelText) => {
              const visible = (el) => {
                const r = el.getBoundingClientRect();
                const style = window.getComputedStyle(el);
                return r.width > 0 && r.height > 0 && style.visibility !== 'hidden' && style.display !== 'none';
              ...
              const inputs = Array.from(document.querySelectorAll('input,textarea'))
                .filter(el => visible(el) && !el.disabled && (el.tagName === 'TEXTAREA' || ['text', 'number', ''].includes(el.type)));
              const containers = Array.from(document.querySelectorAll('.ant-form-item,.form-item,.form-group,tr,li'))
                .filter(visible)
                .filter(el => norm(textOf(el)).includes(norm(labelText)));
              for (const container of containers) {
                const local = Array.from(container.querySelectorAll('input,textarea'))
                  .filter(el => visible(el) && !el.disabled);
                if (local.length) return inputValue(local[0], 'visible_labeled_container');
              }
              const labels = Array.from(document.querySelectorAll('label,span,div,td,th'))
                .filter(visible)
                .map(el => ({el, text: textOf(el), rect: rectOf(el)}))
                .filter(item => item.text && norm(item.text).includes(norm(labelText)) && item.rect.w <= 420 && item.rect.h <= 120);
              for (const label of labels) {
                const lr = label.rect;
                const ly = docY(label.el) + lr.h / 2;
                const rowInputs = inputs
                  .filter(el => {
                    const r = rectOf(el);
                    const y = docY(el) + r.h / 2;
                    return Math.abs(y - ly) < 72 && r.x > lr.x - 20;
                  })
                  .sort((a, b) => rectOf(a).x - rectOf(b).x);
                if (rowInputs.length) return inputValue(rowInputs[0], 'visible_label_row');
              }
              return {ok:false, reason:`未读取到输入框：${labelText}`};
            }''', label_text)
            return state if isinstance(state, dict) else {'ok': False, 'reason': f'未读取到输入框：{label_text}'}
        except Exception as exc:
            return {'ok': False, 'reason': f'读取输入框失败：{label_text} / {exc}'}
    ...
    def _fill_text_inputs_after_label_locator(self, page: Page, label_text: str, values: list[str]) -> dict[str, Any]:
        ...
        container_result = self._fill_text_inputs_in_labeled_container(page, label_text, values)
    ...
    def _fill_text_inputs_in_labeled_container(self, page: Page, label_text: str, values: list[str]) -> dict[str, Any]:
        try:
            result = page.evaluate(r'''({labelText, values}) => {
              const visible = (el) => {
                const r = el.getBoundingClientRect();
                const style = window.getComputedStyle(el);
                return r.width > 0 && r.height > 0 && style.visibility !== 'hidden' && style.display !== 'none';
              ...
              for (const container of containers) {
                ...
                values.forEach((value, index) => {
                  const written = setValue(inputs[index], value);
                  ok = ok && written;
                  if (written) filled.push(String(value ?? ''));
                });
                if (ok) return {ok:true, filled, method:'visible_labeled_container'};
              }
              return {ok:false, reason:`未找到标签容器输入框：${labelText}`};
            }''', {'labelText': label_text, 'values': values})
            if isinstance(result, dict):
                return result
            return {'ok': False, 'reason': f'标签容器填写失败：{label_text}'}
        except Exception as exc:
            return {'ok': False, 'reason': f'标签容器填写异常：{label_text} / {exc}'}
    ...
    def _fill_packaging_info(self, page: Page, gross_weight: str, dimensions: list[str]) -> dict[str, Any]:
        base = page.evaluate(r'''(values) => {
          ...
          const inputs = Array.from(document.querySelectorAll('input,textarea')).filter(visible);
          const byPlaceholder = (text) => inputs.find(el => String(el.placeholder || '') === text && !el.disabled);
          const dimensions = Array.isArray(values.dimensions) ? values.dimensions : [];
          return {
            weight: setValue(byPlaceholder('请输入重量'), values.gross_weight),
            length: setValue(byPlaceholder('长'), dimensions[0]),
            width: setValue(byPlaceholder('宽'), dimensions[1]),
            height: setValue(byPlaceholder('高'), dimensions[2]),
          };
        }''', {'gross_weight': gross_weight, 'dimensions': dimensions})
        if not all(bool(base.get(key)) for key in ('weight', 'length', 'width', 'height')):
            return {'ok': False, 'reason': '基础重量或包装尺寸填写失败', 'base': base}
        ...
        indexes = page.evaluate(r'''(count) => {
          ...
          const allInputs = Array.from(document.querySelectorAll('input,textarea'));
          const gy = docY(gross);
          const grossIndex = allInputs.indexOf(gross);
          const candidates = allInputs
            .filter(el => visible(el) && !el.disabled && el.tagName === 'INPUT' && ['text', 'number', ''].includes(el.type))
            .filter(el => {
              const r = rectOf(el);
              const y = docY(el);
              return allInputs.indexOf(el) > grossIndex && y > gy - 10 && y < gy + 180 && r.w >= 50 && r.w <= 180;
            })
        ...
        page.wait_for_timeout(300)
        return {'ok': True, 'gross_weight': gross_weight, 'dimensions': dimensions, 'base': base}
    ...
    def _click_ant_option_near_rect(
        ...
        if not option or not option.get('rect'):
            reopened = page.evaluate(r'''({anchor}) => {
              const visible = (el) => {
                const r = el.getBoundingClientRect();
                const style = window.getComputedStyle(el);
                return r.width > 0 && r.height > 0 && r.bottom > 0 && r.top < window.innerHeight
              ...
              const textOf = (el) => (el.innerText || el.textContent || '').replace(/\s+/g, ' ').trim();
              const distance = (a, b) => Math.abs((a.x + a.w / 2) - (b.x + b.w / 2)) + Math.abs((a.y + a.h / 2) - (b.y + b.h / 2));
              const selects = Array.from(document.querySelectorAll('.ant-select'))
                .filter(visible)
                .map(el => ({el, text:textOf(el), rect:rectOf(el)}))
                .filter(x => x.rect.x < anchor.x + anchor.w + 80 && x.rect.x + x.rect.w > anchor.x - 80);
              const picked = selects.sort((a, b) => distance(a.rect, anchor) - distance(b.rect, anchor))[0];
              if (!picked) return {ok:false, reason:'select_not_found_near_anchor'};
              const target = picked.el.querySelector('.ant-select-selector') || picked.el;
              ...
              fire('mouseup');
              fire('click');
              return {ok:true, text:picked.text, rect:picked.rect, strategy:'reopen_select_dom_dispatch'};
            }''', {'anchor': anchor_rect}) or {}
            if reopened.get('ok'):
                page.wait_for_timeout(800)
                option = page.evaluate(option_script, {'priorities': priorities, 'anchor': reopened.get('rect') or anchor_rect})
                if option and option.get('rect'):
                    option['strategy'] = option.get('strategy') or reopened.get('strategy')
        if not option or not option.get('rect'):
            dismissed = self._dismiss_blocking_modals(page)
            if dismissed:
    ...
    def _check_choice_by_text(self, page: Page, text: str) -> dict[str, Any]:
        ...
        result.update({
            'located': bool(result.get('rect')),
            'expected_value': text,
            'value_after': text if result.get('checked') is True else None,
        ...
        result = page.evaluate(r'''(values) => {
          ...
          const inputs = Array.from(document.querySelectorAll('input,textarea'))
            ...
            .map(el => ({
              ...
              dataField:el.getAttribute('data-field') || el.closest('[data-field]')?.getAttribute('data-field') || '',
              rect:rectOf(el),
            }));
          ...
          const setOrAccept = (field, item, desiredValue, options = {}) => {
            const detail = {
              ...
              expected_value: String(desiredValue ?? ''),
          ...
          const structuredInput = (aliases) => {
            const expected = aliases.map(value => norm(value));
            const matches = inputs.filter(item => [item.id, item.name, item.dataField]
              .map(value => norm(value))
              .some(value => value && expected.includes(value)));
            return matches.length === 1 ? {...matches[0], strategy:'unique_structured_field_key'} : null;
          };
          const variantTable = tableByTerms(['产品价格', 'JIT库存']);
          const productPrice = structuredInput(['productPrice', 'product_price']);
          const supplyPrice = structuredInput(['supplyPrice', 'supply_price']);
          const weight = structuredInput(['weight', 'packageWeight', 'package_weight']);
          const dimensions = [
            structuredInput(['length', 'packageLength', 'package_length']),
            structuredInput(['width', 'packageWidth', 'package_width']),
            structuredInput(['height', 'packageHeight', 'package_height']),
          ];
          const jitStock = structuredInput(['jitStock', 'jit_stock']);
          const goodsCode = structuredInput(['skuCode', 'sku_code', 'goodsCode', 'goods_code']);
          const goodsBarcode = structuredInput(['goodsBarcode', 'goods_barcode']);
          ...
          const variantRows = variantTable
            ? Array.from(variantTable.querySelectorAll('tbody tr')).filter(visible).map((row, index) => ({
                index,
                structured_control_count: row.querySelectorAll('input[name],textarea[name],select[name],[data-field]').length,
              })).filter(row => row.structured_control_count > 0)
            : [];
          const details = {
    ...
    def _capture_save_field_integrity_snapshot(page: Page) -> dict[str, Any]:
        raw = page.evaluate(r'''() => {
          ...
          const entries = [];
          for (const el of Array.from(document.querySelectorAll('input,textarea,select,[contenteditable="true"]')).filter(visible)) {
            ...
            if (type === 'checkbox' || type === 'radio') {
              ...
              value = clean(el.value || 'checked');
            } else if (el.tagName === 'SELECT') {
              value = clean(el.value || el.selectedOptions?.[0]?.textContent || '');
            ...
            if (!value) continue;
            entries.push({kind:'control', key, type, value});
          }
          for (const el of Array.from(document.querySelectorAll(
            '.ant-select-selection-item,.el-select__selected-item,[data-selected-value]'
          )).filter(visible)) {
            const container = el.closest('.ant-form-item,.el-form-item,[data-field]') || el.parentElement;
            const label = clean(
              container?.querySelector('label,.ant-form-item-label,.el-form-item__label')?.textContent
              || container?.getAttribute('data-field')
              || ''
            );
            const value = clean(el.getAttribute('data-selected-value') || el.textContent || '');
            if (label && value) entries.push({kind:'selected_value', key:label, type:'selection', value});
          }
          for (const el of Array.from(document.querySelectorAll(
            '[data-name],[data-filename],[data-file-name],img[alt],img[title]'
          )).filter(visible)) {
            const value = clean(
            ...
            if (value) entries.push({kind:'asset', key:clean(el.tagName), type:'filename', value});
          }
          for (const el of Array.from(document.querySelectorAll(
            '[data-customs-product-name],[data-customs-kind],[data-product-name]'
          )).filter(visible)) {
            const key = el.hasAttribute('data-customs-product-name')
              ? 'customs_product_name'
              : el.hasAttribute('data-customs-kind') ? 'customs_kind' : 'product_name';
            const value = clean(
              el.getAttribute('data-customs-product-name')
              || el.getAttribute('data-customs-kind')
              || el.getAttribute('data-product-name')
              || el.textContent
              || ''
            );
            if (value) entries.push({kind:'structured_attribute', key, type:'value', value});
          }
        ...
        return {
            'ok': bool(entries and digest),
            'kind': 'structured_nonempty_form_state',
            'field_count': len(entries),
            'nonempty_field_count': len(entries),
            'sha256': digest,
        }
    ...
    def _save_only_on_page(
        ...
        *,
        ...
        self._last_mutation_authorization_facts = None
        if target_identity is None or not str(store_name or '').strip():
            raise FrozenTargetIdentityError(
                'FROZEN_TARGET_REQUIRED',
                '保存操作缺少冻结商品与店铺身份，已停止。',
            )
        ...
        def final_save_target_guard() -> dict[str, Any]:
            identity = self._require_frozen_product_page_identity(
                page,
                target_identity=target_identity,
                store_name=store_name,
                phase='进入 DISPATCHING 前的最终身份与字段读回',
            )
            ...
            integrity_match = bool(
                ...
                and baseline.get('kind') == 'structured_nonempty_form_state'
                and current.get('kind') == baseline.get('kind')
                and int(baseline.get('field_count') or 0) > 0
        ...
        if self._is_visible_dxm_editor_page(page):
            ...
            network_audit = dict(network_capture['network_audit'])
            publish_signal = dict(network_capture['publish_signal'])
            if not clicked:
                self._trace_workflow_event(
            ...
            if not page_ok:
                combined_reason = f"page_save_not_confirmed: {page_result.get('reason') or '未检测到页面保存成功证明'}"
            elif not network_receipt_ok:
                combined_reason = f"network_save_not_confirmed: {network_result.get('reason') or '保存接口未返回成功'}"
            elif not audit_ok:
                combined_reason = 'network_audit_incomplete_or_publish_signal_detected'
            save_result = {
                **save_state,
                **page_result,
                'ok': combined_ok,
                ...
                'network_audit': network_audit,
                'publish_signal': publish_signal,
                'page_save_result': dict(page_result),
                'save_decision': {
                    'ok': combined_ok,
                    'rule': 'page_success_and_network_success',
                    'page_ok': page_ok,
                    'network_ok': network_ok,
                    'network_receipt_ok': network_receipt_ok,
                    'network_audit_ok': audit_ok,
                },
            ...
            return {
                ...
                'message': (
                    save_result.get('success_text') or save_result.get('message') or '保存成功'
                    if ok
                    else save_result.get('reason') or page_result.get('reason') or '保存后未拿到双重成功证明'
                ),
                'page_title': '店小秘编辑页',
                'page_url': page.url,
                'screenshot_url': self._artifact_url(screenshot_path),
        ...
        try:
            ...
            if click_result.get('ok'):
                current_save_status = self._structured_save_status_snapshot(page, visible_browser=False)
                page_result = {
                    **click_result,
        ...
        verify_result = {
            **page_result,
            ...
            'exact_save_target': bool(
                ...
                and click_result.get('exact_save_count') == 1
            ),
            'save_click_dispatched': click_result.get('clicked') is True,
            'network_save_success': network_ok,
            'page_save_success': page_ok,
            'publish_action_clicked': publish_signal.get('detected') is True,
            'published': False if publish_signal.get('detected') is False else None,
        ...
        return {
            ...
            'label': '已保存' if ok else '保存失败',
            'message': (
                verify_result.get('success_text') or verify_result.get('message') or '保存成功'
                if ok
                else verify_result.get('reason') or page_result.get('reason') or '保存失败'
            ),
            'page_title': page.title(),
            'page_url': page.url,
            'screenshot_url': self._artifact_url(screenshot_path),
    ...
    @staticmethod
    def _structured_save_status_snapshot_script() -> str:
        return r'''() => {
          /* __DXM_STRUCTURED_SAVE_STATUS_SNAPSHOT__ */
          const visible = (el) => {
            if (!el || !el.getBoundingClientRect) return false;
            const rect = el.getBoundingClientRect();
            const style = window.getComputedStyle(el);
          ...
          window.__dxmSaveProofSequence = Number(window.__dxmSaveProofSequence || 0);
          const entries = Array.from(new Set(Array.from(document.querySelectorAll(selectors))))
            .filter(visible)
            .map((el) => {
              let id = String(el.getAttribute('data-dxm-save-proof-id') || '');
              if (!id) {
                window.__dxmSaveProofSequence += 1;
                id = `dxm-save-${Date.now()}-${window.__dxmSaveProofSequence}`;
                el.setAttribute('data-dxm-save-proof-id', id);
              }
              const source = el.matches('[data-save-status]') ? 'data-save-status'
                : el.matches('[data-save-state]') ? 'data-save-state'
                : el.getAttribute('role') ? `role-${el.getAttribute('role')}`
                : String(el.className || '').trim().slice(0, 160);
              return {
                id,
                text: textOf(el).slice(0, 500),
                status: String(el.getAttribute('data-save-status') || el.getAttribute('data-save-state') || '').trim().slice(0, 160),
                source,
              };
            });
    ...
    def _structured_save_status_snapshot(self, page: Page, *, visible_browser: bool) -> dict[str, Any]:
        script = self._structured_save_status_snapshot_script()
        try:
            result = (
                self._evaluate_visible_page_function_via_devtools(page, script, timeout=1800)
                if visible_browser
            ...
            return {
                'ok': False,
                'entries': [],
                'reason': f'structured_save_status_probe_failed: {str(exc)[:160]}',
            }
        if not isinstance(result, dict) or not isinstance(result.get('entries'), list):
            return {'ok': False, 'entries': [], 'reason': 'structured_save_status_probe_unreadable'}
        normalized = {'ok': result.get('ok') is True, 'entries': list(result.get('entries') or [])}
        if result.get('reason'):
            normalized['reason'] = str(result.get('reason'))
        return normalized

    @staticmethod
    def _save_status_transition_result(
        ...
        def compact(entry: Any) -> str:
            if not isinstance(entry, dict):
                return ''
            return ''.join(f"{entry.get('text') or ''} {entry.get('status') or ''}".split())
        ...
        current_entries = [entry for entry in current.get('entries') or [] if isinstance(entry, dict)]
        first_error = next(
            (entry for entry in current_entries if any(term in compact(entry) for term in error_terms)),
            None,
        )
        if first_error:
            error_text = str(first_error.get('text') or first_error.get('status') or '')[:500]
            return {
                'ok': False,
                'reason': f'保存后页面提示：{error_text[:240]}',
        ...
        changed = next(
            (
                ...
                if str(entry.get('id') or '') not in baseline_by_id
                or compact(baseline_by_id[str(entry.get('id'))]) != compact(entry)
            ),
            None,
        )
        if not changed:
    ...
    def _visible_save_success_state(
        ...
        *,
        ...
        current = self._structured_save_status_snapshot(page, visible_browser=True)
        result = self._save_status_transition_result(baseline, current)
        self._trace_workflow_event(
    ...
    def _verify_not_published_on_page(
        ...
        *,
        ...
        if target_identity is None or not str(store_name or '').strip():
            raise FrozenTargetIdentityError(
                'FROZEN_TARGET_REQUIRED',
                '未发布校验缺少冻结商品与店铺身份，已停止。',
            )
        identity_readback = self._require_frozen_product_page_identity(
            page,
            target_identity=target_identity,
            store_name=store_name,
            phase='生成独立未发布证据前',
        ...
        result = page.evaluate(r'''() => {
          ...
          const unpublishedStatuses = ['待发布', '草稿', '未发布', '待完善'];
          const publishedStatuses = ['已发布', '发布成功', '已上架', '在线', '在线商品'];
          ...
          const sourceOf = (el) => {
            if (el.hasAttribute('data-product-status')) return 'data-product-status';
            if (el.hasAttribute('data-status')) return 'data-status';
            if (el.hasAttribute('data-state')) return 'data-state';
            return `class:${String(el.className || '').trim().slice(0, 120)}`;
          };
          const candidates = Array.from(new Set(Array.from(document.querySelectorAll(candidateSelector))))
            .filter(visible)
            .map((el) => {
              const statusText = norm(el.getAttribute('data-product-status') || el.getAttribute('data-status') || el.getAttribute('data-state') || textOf(el));
              const scope = el.closest(scopeSelector);
              if (!scope) return null;
              const scopeText = textOf(scope);
              return {
                statusText,
                source: sourceOf(el),
                scopeExcerpt: scopeText.slice(0, 300),
              };
            })
            .filter(item => item && (unpublishedStatuses.includes(item.statusText) || publishedStatuses.includes(item.statusText)));
          const published = candidates.find(item => publishedStatuses.includes(item.statusText)) || null;
          const unpublished = candidates.find(item => unpublishedStatuses.includes(item.statusText)) || null;
          const uniqueBoundStatus = candidates.length === 1;
          const proof = uniqueBoundStatus && !published ? unpublished : null;
          return {
            ok: Boolean(proof),
            proof_kind: proof ? 'structured_unpublished_status' : null,
            status_text: (published || proof)?.statusText || null,
            status_source: (published || proof)?.source || null,
            target_bound: false,
            product_matched: false,
            store_matched: false,
            publish_risk_term: published?.statusText || null,
            structured_candidate_count: candidates.length,
            bound_candidate_count: candidates.length,
        ...
        source_matched = identity_readback.get('source_identity_match') is True
        target_bound = bool(product_matched and store_matched and source_matched)
        result.update({
            ...
            # Compatibility alias for older diagnostic consumers.  The
            # canonical VERIFY contract is source_identity_match.
            'source_matched': source_matched,
            'identity_readback': dict(identity_readback),
        })
        normalized_status = ''.join(str(result.get('status_text') or '').split())
        unpublished_statuses = {'待发布', '草稿', '未发布', '待完善'}
        proof_valid = (
        ...
        result['identity_binding_kind'] = 'frozen_target_structured_page_readback'
        if not proof_valid and not result.get('reason'):
            result['reason'] = 'structured_unpublished_status_missing'
        evidence_ref = self._capture_scoped_evidence_screenshot(
            page,
            SCREENSHOT_DIR / 'dianxiaomi_verify_not_published.png',
        ...
        return {
            ...
            'message': result.get('status_text') or result.get('publish_risk_term') or result.get('reason') or '未找到当前商品的结构化未发布证明。',
            'page_title': page.title(),
            'page_url': page.url,
            'screenshot_url': self._artifact_url(screenshot_path),
            'evidence_ref': evidence_ref,
    ...
    def _capture_save_network_events(self, page: Page, save_rect: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if current.scheme not in {'http', 'https'} or not current.netloc:
            return session
        session['page_origin'] = f'{current.scheme.lower()}://{current.netloc.lower()}'
        events: list[dict[str, Any]] = session['events']
        ...
        def on_request(request) -> None:
            ...
            mutation_kind = self._same_origin_mutation_request_kind(
                page_origin=str(session['page_origin']),
                request_url=url,
                method=method,
            )
        ...
        try:
            ...
            return session
        session['listeners'].append(('request', on_request))
        try:
            page.on('response', on_response)
        except Exception:
            return session
        session['listeners'].append(('response', on_response))
        session['registered'] = True
        return session

    @staticmethod
    def _same_origin_mutation_request_kind(
        *,
        page_origin: str,
        request_url: str,
        method: str | None,
    ...
    def _finalize_save_network_audit(page: Page, session: Mapping[str, Any]) -> dict[str, Any]:
        listeners = list(session.get('listeners') or [])
        removed = 0
        for event_name, listener in listeners:
            remover = getattr(page, 'remove_listener', None)
            if not callable(remover):
                remover = getattr(page, 'off', None)
            if not callable(remover):
                continue
            try:
                remover(event_name, listener)
            except Exception:
                continue
            removed += 1
        ...
        detected_kind = (
            ...
            if publish_requests
            else 'network_route_classification'
        )
        return {
            'events': events,
            'network_audit': {
    ...
    def _close_browser_session(self) -> None:
        # Revoke the authorization generation before any potentially failing close call.
        self._clear_browser_context_generation()
    ...
    def _select_devtools_page_target(self, page: Page, targets: list[Any]) -> dict[str, Any] | None:
        ...
        exact_matches = [
            ...
            and item.get('type') == 'page'
            and item.get('webSocketDebuggerUrl')
            and str(item.get('url') or '').split('#', 1)[0] == current_url_no_hash
        ]
        return exact_matches[0] if current_url_no_hash and len(exact_matches) == 1 else None
    ...
    def _select_browser_process_id(
        self,
        ...
        *,
        ...
        expected_profile = os.path.normcase(os.path.normpath(str(Path(profile_dir).resolve())))
        matches: list[int] = []
        for record in records:
            ...
            if self._remote_debugging_port_from_command_line(command_line) != int(port):
                continue
            raw_profile = self._command_line_user_data_dir(command_line)
            if not raw_profile:
                continue
            actual_profile = os.path.normcase(os.path.normpath(str(Path(raw_profile).resolve())))
            ...
            try:
                ...
                continue
            if pid > 0 and pid not in matches:
                matches.append(pid)
        if popen_pid is not None:
            try:
                expected_pid = int(popen_pid)
            except (TypeError, ValueError):
                return None
            matches = [pid for pid in matches if pid == expected_pid]
        return matches[0] if len(matches) == 1 else None

    def _browser_process_records(self) -> list[dict[str, Any]]:
        if os.name != 'nt':
            return []
        script = (
    ...
    def _resolve_native_browser_identity(self, page: Page) -> dict[str, Any] | None:
        if page is not self._page or self._context is None or self._browser is None:
            return None
        ...
        port = self._remote_debugging_port
        if not page_url or not port:
            return None
        try:
            targets = self._devtools_json('/json/list', timeout_s=1.0)
        except Exception:
            return None
        target = self._select_devtools_page_target(page, targets if isinstance(targets, list) else [])
        ...
        if process is not None:
            try:
                if process.poll() is None:
                    popen_pid = int(process.pid)
            except Exception:
                return None
        browser_pid = self._select_browser_process_id(
            self._browser_process_records(),
            port=int(port),
            profile_dir=self._workflow_browser_profile_dir(),
            popen_pid=popen_pid,
        )
    ...
    def _wait_for_body_text(
        ...
        *,
        ...
        if identity_result.get('ok') is not True:
            self._trace_workflow_event(
                'body_text_wrong_route',
                terms=terms,
                expected_identity=expected_identity,
                identity_result=identity_result,
                current_url=getattr(page, 'url', None),
                human_step='目标页路由校验失败',
            )
            return False
        if self._is_visible_dxm_editor_page(page):
        ...
        try:
            page.wait_for_function(
                r"""(terms) => {
                  ...
                  const markerNodes = Array.from(document.querySelectorAll(
                    'h1,h2,h3,label,button,[role="button"],[role="tab"],.ant-tabs-tab,.el-tabs__item,table thead th,.vxe-header--column,.ant-table-thead th,input[placeholder],textarea[placeholder],[class*="form-item"],[class*="FormItem"]'
                  )).filter(visible);
                  const markerTexts = markerNodes.map(el => String(
                    el.innerText || el.textContent || el.getAttribute('placeholder') || el.getAttribute('aria-label') || ''
                  ).replace(/\s+/g, ' ').trim()).filter(Boolean);
                  const businessMarker = terms.find(term => markerTexts.some(text => text.includes(term)));
                  const loadingOverlay = Array.from(document.querySelectorAll(
                    '.ant-spin-spinning,.vxe-loading,.vxe-loading--wrapper,.el-loading-mask,.loading,[class*="loading"],[class*="Loading"]'
                  )).some(visible);
                  return Boolean(businessMarker) && !loadingOverlay;
                }""",
                arg=terms,
                timeout=timeout,
            )
    ...
    def _wait_for_body_text_with_runtime(
        self,
        page: Page,
        terms: list[str],
        timeout: int = 20000,
        *,
        ...
        script = r'''(terms) => {
          ...
          const readyTerm = (terms || []).find(term => markerTexts.some(text => text.includes(String(term || '')))) || null;
          ...
          return {
            ...
            loading,
            text_excerpt: markerTexts.slice(0, 12).join(' | ').slice(0, 300),
            url: location.href,
          };
        }'''
        while True:
    ...
    def _wait_for_page_ready(
        ...
        *,
        ...
        if identity_result.get('ok') is not True:
            raise RuntimeError(
                f'{label} 页面路由不匹配 expected_identity={expected_identity} '
                f'url={getattr(page, "url", None)}'
            )
        self._trace_workflow_event('wait_ready:start', label=label, terms=terms, expected_identity=expected_identity, timeout=timeout, current_url=getattr(page, 'url', None), dismiss_strategy=dismiss_strategy)
        deadline = time.monotonic() + timeout / 1000
        last: dict[str, Any] = {}
        last_trace_at = 0.0
        ...
        while time.monotonic() < deadline:
            ...
            if data_acquisition_scan:
              ...
              const markerTexts = markerNodes.map(el => (
                el.innerText || el.textContent || el.getAttribute('placeholder') || ''
              ).replace(/\s+/g, ' ').trim()).filter(Boolean);
              const readyTerm = terms.find(term => markerTexts.some(text => text.includes(term)));
              const loadingNodes = Array.from(document.querySelectorAll(
                '.ant-spin-spinning, .vxe-loading, .vxe-loading--wrapper, .el-loading-mask, .loading, [class*="loading"], [class*="Loading"]'
              )).filter(visible);
              const loadingText = loadingNodes
                ...
                .join('');
              const loading = loadingNodes.length > 0;
              ...
              return {
                ...
                business_marker: readyTerm || null,
                business_marker_count: markerTexts.filter(text => terms.some(term => text.includes(term))).length,
                loading,
                loading_count: loadingNodes.length,
                rows,
                inputs,
                text_excerpt: markerTexts.slice(0, 20).join(' | ').slice(0, 500),
                url: location.href,
                title: document.title,
                loading_text: loadingText.slice(0, 200),
              };
    ...
    def _search_draft_box(self, page: Page, product_query: str | None = None, store_name: str | None = None) -> None:
        ...
        if visible_draft_box and not str(product_query or '').strip():
            self._trace_workflow_event(
                'draft_box_search:submit_skipped_visible_empty_query',
                store_name=store_name,
                reason='visible_browser_store_only_search_would_reset_list',
                human_step='可见浏览器跳过空商品箱搜索',
            )
            return
    ...
    def _settle_visible_draft_box(self, page: Page, *, expected_identity: str) -> dict[str, Any]:
        self._trace_workflow_event(
        ...
        result = {
            ...
            'url': getattr(page, 'url', None),
            'title': readiness.get('page_title') or '',
            'read_source': 'browser_readiness_gate',
            'read_error': '' if readiness.get('ok') else readiness.get('reason') or 'page_not_ready',
            'expected_identity': expected_identity,
            'readiness': readiness,
        }
        self._trace_workflow_event(
    ...
    def _inspect_visible_draft_box_state(self, page: Page) -> dict[str, Any]:
        ...
        return {
            ...
            'inputs': None,
            'text_excerpt': readiness.get('body_excerpt') or '',
            'url': readiness.get('page_url') or getattr(page, 'url', None),
            'title': readiness.get('page_title') or '',
            'read_source': 'browser_readiness_gate',
            'read_error': '' if readiness.get('ok') else readiness.get('reason') or 'page_not_ready',
        }
    ...
    def _submit_visible_draft_box_search(self, page: Page, product_query: str) -> dict[str, Any]:
        return page.evaluate(r'''(frag) => {
          ...
          if (valueSetter) {
            valueSetter.call(input, frag || '');
          } else {
            input.value = frag || '';
          }
          if (window.InputEvent) {
            input.dispatchEvent(new InputEvent('input', {bubbles:true, inputType:'insertText', data: frag || ''}));
          } else {
            input.dispatchEvent(new Event('input', {bubbles:true}));
          }
          input.dispatchEvent(new Event('change', {bubbles:true}));
          const clickableSelector = 'button,a,[role="button"],input[type="button"],input[type="submit"]';
          const exactControls = Array.from(document.querySelectorAll(clickableSelector))
            .filter(visible)
            .map(el => {
          ...
          const fallbackControls = exactControls.length ? [] : Array.from(document.querySelectorAll('span,div'))
            .filter(visible)
            .map(el => {
              const target = el.closest(clickableSelector);
              if (!target) return null;
              const text = norm(target.innerText || target.textContent || target.value || target.getAttribute('aria-label') || target.getAttribute('title'));
              const tr = target.getBoundingClientRect();
              return {
                target,
                text,
            ...
            .filter(Boolean)
            .filter(item => item.text === '搜索' || item.text.endsWith('搜索'));
          const controls = exactControls.length ? exactControls : fallbackControls;
          const button = controls.sort((a, b) => {
            const aExact = a.text === '搜索' ? 0 : 1;
            const bExact = b.text === '搜索' ? 0 : 1;
          ...
          if (button) {
            ...
            return {ok:true, strategy:'dom_visible_input_click_button', input:describe(input), clicked:button.text, value:String(input.value || '')};
          }
          input.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', bubbles:true, cancelable:true}));
          input.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', code:'Enter', bubbles:true, cancelable:true}));
          return {ok:true, strategy:'dom_visible_input_enter', input:describe(input), clicked:null, value:String(input.value || '')};
        }''', product_query)
    ...
    def _find_draft_box_row(
        ...
        try:
          ...
          const looksTableWrapper = (text) => (
            /图片\s*标题\/产品ID/.test(text)
            && /分组\s*价格\s*库存/.test(text)
            && text.includes('操作')
          );
          const looksAggregate = (text) => (
            (text.match(/创建：/g) || []).length > 1
            || (text.match(/移入待发布/g) || []).length > 2
            || looksTableWrapper(text)
          );
          const sourceUrls = (row) => Array.from(row.querySelectorAll('a[href]'))
            .map(a => String(a.href || a.getAttribute('href') || ''))
          ...
          const sourceUrlMatches = (url, target) => {
            const actual = canonicalUrl(url);
            const expected = canonicalUrl(target);
            return Boolean(actual && expected && actual === expected);
          };
          const storeEvidence = (row) => {
            if (!store) return null;
            const storeNorm = String(store).replace(/\s+/g, '').trim().toLocaleLowerCase();
            const cells = Array.from(row.querySelectorAll('td,[role="cell"],.vxe-body--column,.ant-table-cell,.el-table__cell,[class*="cell"]'))
              .map(cell => ({cell, text:normText(cell)}))
              .filter(item => {
                const compact = item.text.replace(/\s+/g, '').trim().toLocaleLowerCase();
                return compact === storeNorm
                  || compact === `「${storeNorm}」`
                  || compact.includes(`「${storeNorm}」`);
              })
              .sort((a, b) => a.text.length - b.text.length);
            const picked = cells[0];
            if (!picked) return null;
            return {
              store_name:String(store).replace(/\s+/g, ' ').trim(),
              cell_text:picked.text.slice(0, 240),
              source:'structured_store_cell',
              tag:String(picked.cell.tagName || ''),
          ...
          const candidates = rows.map((tr, idx) => ({idx, text:normText(tr)})).filter(x => {
            ...
            if (store && !storeEvidence(rows[x.idx])) return false;
            if (targetUrls.length) return hasTargetSource(rows[x.idx]);
          ...
          if (claimMatches.length === 1) {
            ...
            return {ok:true, rowIndex:claimMatches[0].idx, rowText:claimMatches[0].text.slice(0,700), sourceUrls:sourceUrls(row), storeEvidence:storeEvidence(row), actions, matchedBy:'claim_mark'};
          }
        ...
        try:
            result = self._evaluate_page_function_with_runtime_timeout(page, r'''({frag, store, claimMark, targetSourceUrls}) => {
              ...
              const storeEvidence = (row) => {
                ...
                const storeNorm = String(store).replace(/\s+/g, '').trim().toLocaleLowerCase();
                const cells = Array.from(row.querySelectorAll('td,[role="cell"],.vxe-body--column,.ant-table-cell,.el-table__cell,[class*="cell"]'))
                  .map(cell => ({cell, text:textOf(cell)}))
                  .filter(item => {
                    const compact = item.text.replace(/\s+/g, '').trim().toLocaleLowerCase();
                    return compact === storeNorm
                      || compact === `「${storeNorm}」`
        ...
        *,
        ...
        for selector in selectors:
            ...
            for index in range(count):
                ...
                if not row_text:
                    continue
                store_evidence = self._structured_store_cell_evidence_in_container(row, store_name)
                if store_name and store_evidence is None:
                    continue
                ...
                if not action_href and not self._rect_has_clickable_area(action_rect):
                    ...
                    continue
                source_urls = self._source_urls_in_container(row)
                if target_source_urls and not self._source_urls_match(source_urls, target_source_urls):
                    samples.append({'selector': selector, 'index': index, 'skip': 'authorized_source_missing', 'rowText': row_text[:160]})
                    continue
                return {
                    'ok': True,
                    'rowIndex': index,
    ...
    def _click_rect_center(
        ...
        *,
        ...
        cdp_clicked = (
            ...
            if mutation_action
            else self._click_point_with_cdp(page, x, y)
        )
        if cdp_clicked:
            self._trace_workflow_event('click_rect:done', method='cdp', human_step='点击页面按钮')
            return
        native_clicked = (
            self._click_point_with_native_window(
                page,
                x,
                y,
                mutation_action=mutation_action,
            ...
            if mutation_action
            else self._click_point_with_native_window(page, x, y)
        )
        if native_clicked:
        ...
        try:
            ...
            authorization = (
                self._dispatch_authorized_mutation(
                    mutation_action,
                    mouse_click_operation,
                    pre_dispatch_guard=pre_dispatch_guard,
                )
                if mutation_action
                else None
            )
            operation_result = authorization.get('operation_result') if authorization else mouse_click_operation()
            if operation_result is not True:
                raise MutationAuthorizationError(
                    'MUTATION_OPERATION_RESULT_INVALID: Playwright mouse operation 未确认成功'
                )
            self._trace_workflow_event(
                'click_rect:done',
    ...
    def _click_data_acquisition_claim_rect_center(
        self,
        page: Page,
        rect: dict[str, Any],
        *,
        ...
        if os.name == 'nt' and not self._is_headless() and self._is_data_acquisition_page_url(page):
            ...
            try:
                ...
                operation_result = authorization.get('operation_result') if authorization else click_operation()
                if not isinstance(operation_result, Mapping) or operation_result.get('dispatched') is not True:
                    raise MutationAuthorizationError(
                        'MUTATION_OPERATION_RESULT_INVALID: 认领点击 operation 未确认 dispatched'
                    )
                self._trace_workflow_event(
        ...
        return {
            ...
            'method': 'rect_center',
            'authorization': (
                dict(self._last_mutation_authorization_facts or {})
                if mutation_action
                else None
            ),
        }
    ...
    @staticmethod
    def _clamp_native_content_rect(
        content_rect: dict[str, Any],
        *,
        window_rect: dict[str, Any] | None = None,
        virtual_screen: dict[str, Any] | None = None,
    ) -> dict[str, int]:
        def number(source: dict[str, Any] | None, key: str, default: int = 0) -> int:
            if not isinstance(source, dict):
                return default
            try:
                return int(round(float(source.get(key))))
            except (TypeError, ValueError):
                return default

        left = number(content_rect, 'left')
        top = number(content_rect, 'top')
        right = number(content_rect, 'right', left + number(content_rect, 'width'))
        bottom = number(content_rect, 'bottom', top + number(content_rect, 'height'))

        if isinstance(window_rect, dict):
            window_left = number(window_rect, 'left', left)
            window_top = number(window_rect, 'top', top)
            window_right = number(window_rect, 'right', window_left + number(window_rect, 'width'))
            window_bottom = number(window_rect, 'bottom', window_top + number(window_rect, 'height'))
            left = max(left, window_left)
            top = max(top, window_top)
            right = min(right, window_right)
            bottom = min(bottom, window_bottom)

        if isinstance(virtual_screen, dict):
            screen_left = number(virtual_screen, 'left', left)
            screen_top = number(virtual_screen, 'top', top)
            screen_right = screen_left + number(virtual_screen, 'width')
            screen_bottom = screen_top + number(virtual_screen, 'height')
        ...
        if right <= left or bottom <= top:
            width = max(0, number(content_rect, 'width', right - left))
            height = max(0, number(content_rect, 'height', bottom - top))
            return {
                'left': number(content_rect, 'left'),
                'top': number(content_rect, 'top'),
                'right': number(content_rect, 'left') + width,
                'bottom': number(content_rect, 'top') + height,
                'width': width,
                'height': height,
            }
        return {
    ...
    @staticmethod
    def _screen_point_inside_virtual_screen(point: dict[str, Any], virtual_screen: dict[str, Any] | None) -> bool:
        if not isinstance(virtual_screen, dict):
            return True
        try:
            x = int(point.get('x'))
            y = int(point.get('y'))
            left = int(virtual_screen.get('left') or 0)
            top = int(virtual_screen.get('top') or 0)
            width = int(virtual_screen.get('width') or 0)
            height = int(virtual_screen.get('height') or 0)
        except (TypeError, ValueError):
            return False
        if width <= 0 or height <= 0:
            return False
    ...
    def _select_owned_native_window(
        self,
        ...
        *,
        ...
        exact_owned = [
            ...
            and int(candidate.get('pid') or 0) == int(expected_pid)
            and str(candidate.get('class') or '').startswith('Chrome_WidgetWin_')
            and self._window_position_match_score(candidate.get('rect') or {}, viewport_metrics) >= 80
        ]
        return exact_owned[0] if len(exact_owned) == 1 else None

    @staticmethod
    def _native_binding_snapshot_matches(
    ...
    def _force_foreground_bound_window(self, hwnd: int, expected_pid: int, user32: Any) -> bool:
        try:
            import ctypes
            from ctypes import wintypes

            pid_value = wintypes.DWORD()
            user32.GetWindowThreadProcessId(wintypes.HWND(int(hwnd)), ctypes.byref(pid_value))
            if int(pid_value.value) != int(expected_pid) or not user32.IsWindowVisible(wintypes.HWND(int(hwnd))):
                return False
            user32.ShowWindow(wintypes.HWND(int(hwnd)), 9)
            user32.BringWindowToTop(wintypes.HWND(int(hwnd)))
            ...
            user32.GetWindowThreadProcessId(wintypes.HWND(int(hwnd)), ctypes.byref(current_pid))
            return foreground == int(hwnd) and int(current_pid.value) == int(expected_pid)
        except Exception:
            return False

    def _current_native_binding_snapshot(
        self,
        ...
        *,
        ...
        try:
            ...
            pid_value = wintypes.DWORD()
            user32.GetWindowThreadProcessId(wintypes.HWND(int(window_hwnd)), ctypes.byref(pid_value))
            foreground_hwnd = self._window_handle_to_int(user32.GetForegroundWindow())
        except Exception:
            pid_value = type('_Pid', (), {'value': 0})()
            foreground_hwnd = 0
        target_id = None
        try:
            targets = self._devtools_json('/json/list', timeout_s=0.8)
        ...
        return {
            'session_id': self.browser_session_id(),
            'browser_id': id(self._browser) if self._browser is not None else None,
            'context_id': id(self._context) if self._context is not None else None,
            'page_id': id(page) if page is self._page else None,
            'page_url': str(getattr(page, 'url', '') or '').split('#', 1)[0],
            'target_id': target_id,
            'browser_pid': int(pid_value.value),
    ...
    def _window_restore_succeeded(
        ...
        *,
        ...
        before_needs_restore = self._window_needs_restore_to_primary_screen(
            before_rect,
            virtual_screen=virtual_screen,
        )
        after_needs_restore = self._window_needs_restore_to_primary_screen(
            after_rect,
            virtual_screen=virtual_screen,
        )
        return {
            'ok': not after_needs_restore,
            'before_needs_restore': before_needs_restore,
            'after_needs_restore': after_needs_restore,
    ...
    def _force_foreground_dxm_window(self) -> bool:
        ...
        try:
            ...
            foreground = self._window_handle_to_int(user32.GetForegroundWindow())
            after_rect_raw = Rect()
            user32.GetWindowRect(hwnd_value, ctypes.byref(after_rect_raw))
            after_window_rect = {
                'left': int(after_rect_raw.left),
                'top': int(after_rect_raw.top),
                'width': int(after_rect_raw.right - after_rect_raw.left),
                'height': int(after_rect_raw.bottom - after_rect_raw.top),
            }
            restore_check = self._window_restore_succeeded(
                window_rect,
                after_window_rect,
                virtual_screen=virtual_screen,
            )
            ok = foreground == hwnd and restore_check.get('ok') is True
            self._trace_workflow_event(
                ...
                restored_window_rect=after_window_rect,
        ...
        *,
        ...
        if not binding:
            self._trace_workflow_event(
                ...
                reason='browser_identity_unbound',
                human_step='点击页面按钮',
            )
            return False
        try:
            ...
            target = self._select_owned_native_window(
                windows,
                expected_pid=int(binding['browser_pid']),
                viewport_metrics=viewport_metrics,
            )
            if target is None:
                self._trace_workflow_event(
                    'click_rect:native_click_skipped',
                    reason='owned_browser_window_not_unique',
                    expected_pid=binding.get('browser_pid'),
                    human_step='点击页面按钮',
                )
                return False
            if not self._force_foreground_bound_window(
                int(target['hwnd']),
                int(binding['browser_pid']),
                user32,
            ):
                self._trace_workflow_event(
            ...
            def collect_child(child_hwnd, _lparam):
                ...
                child_hwnd_int = self._window_handle_to_int(child_hwnd)
                if child_hwnd_int:
                    child_rects.append({'hwnd': child_hwnd_int, 'class': class_name, 'rect': rect})
                return True
            ...
            if len(child_rects) != 1:
                self._trace_workflow_event(
                    'click_rect:native_click_skipped',
                    reason='render_widget_not_unique',
                    title=str(target.get('title') or '')[:160],
                    human_step='点击页面按钮',
                )
                return False
            content = child_rects[0]
            virtual_screen = self._win32_virtual_screen(user32)
            raw_content_rect = content['rect']
            content_rect = self._clamp_native_content_rect(
                raw_content_rect,
                window_rect=target.get('rect') if isinstance(target.get('rect'), dict) else None,
                virtual_screen=virtual_screen,
            )
            if content_rect != raw_content_rect:
                self._trace_workflow_event(
                    'click_rect:native_content_rect_clamped',
                    raw_content_rect=raw_content_rect,
                    content_rect=content_rect,
                    window_rect=target.get('rect'),
                    virtual_screen=virtual_screen,
                    human_step='校准浏览器窗口位置',
                )
            ...
            if not self._screen_point_inside_virtual_screen(point['screen'], virtual_screen):
                self._trace_workflow_event(
                    'click_rect:native_click_skipped',
                    reason='screen_point_outside_virtual_screen',
                    screen={'x': screen_x, 'y': screen_y},
                    virtual_screen=virtual_screen,
                    content_rect=content_rect,
                    viewport=viewport_metrics,
            ...
            expected_snapshot = {
                **binding,
                'window_hwnd': int(target['hwnd']),
                'foreground_hwnd': int(target['hwnd']),
            }
            actual_snapshot = self._current_native_binding_snapshot(
                binding,
                page,
                window_hwnd=int(target['hwnd']),
                user32=user32,
            )
            if not self._native_binding_snapshot_matches(expected_snapshot, actual_snapshot):
                self._trace_workflow_event(
                    'click_rect:native_click_skipped',
                    reason='browser_binding_changed_before_dispatch',
                    expected_binding=expected_snapshot,
                    actual_binding=actual_snapshot,
                    human_step='点击页面按钮',
                )
                return False
            ...
            authorization = (
                self._dispatch_authorized_mutation(
                    mutation_action,
                    native_click_operation,
                    pre_dispatch_guard=pre_dispatch_guard,
                )
                if mutation_action
                else None
            )
            operation_result = authorization.get('operation_result') if authorization else native_click_operation()
            if operation_result is not True:
                raise MutationAuthorizationError(
                    'MUTATION_OPERATION_RESULT_INVALID: Win32 mouse_event operation 未确认成功'
                )
            self._trace_workflow_event(
                'click_rect:native_click_done',
                authorization=(authorization or {}).get('authorization_facts'),
                human_step='点击页面按钮',
            )
            return True
    ...
    def _click_point_with_cdp(
        ...
        *,
        ...
        try:
            ...
            moved_event = {'type': 'mouseMoved', 'x': x, 'y': y, 'button': 'none'}
            self._trace_workflow_event(
                'click_rect:cdp_send_start',
                mouse_event='mouseMoved',
                human_step='点击页面按钮',
            )
            cdp.send('Input.dispatchMouseEvent', moved_event)
            self._trace_workflow_event(
            ...
            operation_result = authorization.get('operation_result') if authorization else cdp_click_operation()
            if operation_result is not True:
                raise MutationAuthorizationError(
                    'MUTATION_OPERATION_RESULT_INVALID: CDP mouse operation 未确认成功'
                )
            self._trace_workflow_event(
                'click_rect:cdp_authorized_dispatch_done',
                authorization=(authorization or {}).get('authorization_facts'),
                human_step='点击页面按钮',
            )
            return True
