...
export function HomePage({ workspace, selectedTask, configPreview, runtimeStatus, onShowDxmAccess, onShowTasks, onShowConfig, onShowConsole, onShowReports }: HomePageProps) {
  ...
  const nextAction = selectedTaskCompleted
    ...
    : !dxmLoggedIn
      ...
      : !selectedTask
        ? { label: '创建单商品只保存任务', detail: '选择一个商品，只创建受控保存任务。', cta: '去商品与任务', action: onShowTasks }
        : !configReady
          ? { label: '补齐编辑页配置', detail: '确认本次任务执行会读取哪些字段值。', cta: '去补配置', action: onShowConfig }
          : !l2Passed
            ? { label: '运行真实只读检查', detail: '先读取店小秘页面，确认不会领取、保存或发布。', cta: '去运行只读检查', action: onShowConsole }
            : requiresManualApproval(selectedTask) && !l3Passed
              ? { label: '人工确认只保存', detail: '确认只保存、不发布后才能启动执行浏览器。', cta: '去人工确认', action: onShowConsole }
              : { label: '启动执行浏览器', detail: '系统将操控独立真实浏览器，只保存当前单商品。', cta: '进入真实浏览器', action: onShowConsole }
  const homeStatusCards = [
    ...
    { label: '编辑页配置', value: configReady ? '已就绪' : '待补齐', ok: configReady, detail: configReady ? '执行取值已可核对' : '按店小秘编辑页分区补字段' },
  ...
  return (
    <section className="dashboard-grid" aria-label="首页">
      <div className="hero-panel home-command">
        <div>
          <h1>店小秘半托管只保存自动化</h1>
          <p>按真实店铺、真实商品和编辑页配置推进：登录店小秘、选择任务、补配置、只读检查、人工确认后，只执行单商品保存。</p>
