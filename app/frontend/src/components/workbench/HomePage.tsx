...
export function HomePage({ workspace, selectedTask, configPreview, runtimeStatus, onShowDxmAccess, onShowAcquisition, onShowDraftEdit, onShowTasks, onShowConfig, onShowConsole, onShowReports }: HomePageProps) {
  ...
  const nextAction = claimTaskCompleted
    ? { label: '选择采集箱商品并编辑保存', detail: '选择已进入采集箱的真实商品，确认模板后只保存。', cta: '去采集箱商品', action: onShowDraftEdit }
    ...
    : !dxmLoggedIn
      ...
      : !selectedTask
        ? { label: '开始采集认领', detail: '先从数据采集把真实商品认领到采集箱。', cta: '去采集认领', action: onShowAcquisition }
        : selectedTask.mode === 'claim_only' && selectedTask.status !== 'completed'
          ? { label: '运行采集认领', detail: '打开真实浏览器，把商品认领到采集箱。', cta: '去真实浏览器', action: onShowConsole }
        : !configReady
          ? { label: '检查编辑页模板', detail: '确认本次编辑保存会读取哪套模板和字段值。', cta: '去编辑页模板', action: onShowConfig }
          : !l2Passed
            ? { label: '运行保存前安全检查', detail: '先读取店小秘页面，确认不会领取、保存或发布。', cta: '去真实浏览器', action: onShowConsole }
  ...
  const decisionCards = [
    ...
    { label: '下一步', value: nextAction.cta, detail: selectedTask ? displayTaskName(selectedTask) : '从采集认领开始。', ok: true },
  ]
  ...
  return (
    <section className="dashboard-grid" aria-label="今天做什么">
      <div className="hero-panel home-command">
        <div>
          <h1>操作首页</h1>
          <p>按两段真实流程推进：先采集认领到采集箱，再从采集箱商品进入编辑保存。</p>
...
function OperationGuide({ workspace, selectedTask }: { workspace: DeliveryWorkspace; selectedTask: Task | null }) {
  ...
  const steps = [
    ...
    { label: '第一段：采集认领到采集箱', ok: selectedTask?.mode === 'claim_only' || selectedTask?.mode === 'single_save' },
    { label: '第二段：采集箱商品编辑保存', ok: selectedTask?.mode === 'single_save' },
