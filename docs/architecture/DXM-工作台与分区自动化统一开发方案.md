...
| v1.1.1 | 2026-08-26 | OpenAI GPT（Codex）+ 用户裁决 | 一致性修订：固定店小秘原生半托管资格门、修正 Path B 时序、唯一 Runner、inspect effect class、长动作授权、cancel/UNKNOWN、草稿 revision checkpoint、三层幂等身份、legacy 只读 Adapter、静态 reason code 与反向 DoD；清除乱码 | [v1.1.1 归档](_archive/DXM-工作台与分区自动化统一开发方案.v1.1.1.md) |
| v1.1.2 | 2026-08-26 | OpenAI GPT（Codex）+ 用户裁决 | 安全收敛：单写者围栏、inspect effect 观测合同、固定 ContentFinalize 次序、不可放宽的 snapshot 执行约束、resolving/HVD、canonical serialization、direct `editFromSmt` 证据门与静态 reason code | 当前 |
...
v1.1.2 只修正文档合同，不宣称对应代码已实现，也不放行真实保存或发布：

1. 同一店铺只允许一个系统内 writer；围栏使用事务 CAS 和 fencing token，但外部浏览器无法被全局观察，仍以实时 revision、页面和 Schema 漂移 fail-closed。
2. `inspect` 不再按 DOM 标签或 HTTP method 猜副作用；所有动作必须来自版本化 allowlist，记录前后状态并以实际观察到的最严重 effect 裁决。
3. `ContentFinalize` 固定为 `video → wholesale → translation`，每步后执行 HVD checkpoint，最后统一 delegated-field readback。
4. snapshot 执行约束不可由运营覆盖：过期、类目漂移、Schema 漂移、catalog 漂移或草稿缺失均在任何写入前中止。
5. `resolving` 是证据尚未闭合的黄色非终态；暂停延后生效，继续禁用，停止/急停只记录意图并阻止新命令，不得伪造成功、失败或 UNKNOWN。
6. `plan_content_sha256` 使用跨语言 canonical serialization；direct `editFromSmt` 在取得版本化真实证据前保持 `DISABLED_UNTIL_OBSERVED_EVIDENCE_VERSION`。
...
- `OBSERVED`：视频、批发、翻译、半托管和回滚已有若干类、配置或测试 seam，但尚未共同贯穿正式 Runner、BrowserAgent、ledger 和回执。
...
- `NEEDS_LIVE_EVIDENCE`：店小秘是否可能在原生门后不经过已实证中间 Modal 而直接到达正式 `editFromSmt`。该候选路线当前不可执行；首次现场出现时只收集脱敏 Evidence Pack、停批并走版本化裁决。
...
- 同一时刻、同一 `shop_id` 只允许一个系统内部正式写 task 持有 writer fence；`ConcurrentEditorGuard` 是校验器，不是第二个 Runner、Dispatcher 或任务队列。
- 围栏至少绑定 `writer_fence_id`、`shop_id`、`owner_task_id`、`execution_attempt_id`、`generation`、`acquired_at`、`heartbeat_at` 和 `expires_at`。获取、续心跳、所有权校验、转移和释放均使用事务 CAS；每个外部 action 或 SAVE 的 ledger `BEGIN` 必须再次核对当前 generation/fencing token。
- writer fence 在 task start 时获取，不在 freeze 时长期占用。暂停可继续持有；已证明零派发的停止、明确成功或安全终态才可释放。已派发事实不确定时进入人工复核隔离，禁止自动释放后启动同店新 writer。
- 系统只能强制约束自身托管的 writer。另一个浏览器、手工编辑或外部工具无法被全局可靠观察，因此不得宣称“已排除所有人工写者”；每个页面首写、长动作、入口动作和 SAVE 前仍必须复核服务器 revision、页面身份、类目和 Schema。
- 崩溃恢复只有在 ledger 能证明零派发时才可释放孤儿围栏；无法证明时保持人工复核隔离。旧 generation 永久失效，禁止续用。
...
1. 复核 task、当前 running job、前序成功/后序 pending、queue version、approval、account/session/shop、writer fencing token 和冻结商品身份；视频/翻译/入口转换即时签发并复核一次性 `action_grant_id`，SAVE 即时签发并复核一次性 `save_lease_id`。
...
canonical V1TaskRunner
  -> ConcurrentEditorGuard（同店铺 writer fence；不拥有调度或派发权）
...
| `ConcurrentEditorGuard`       | `acquire/heartbeat/validate/release`     | 以事务 CAS 管理同店铺系统内 writer fence；外部编辑仍由实时漂移检查拦截 |
...
5. **同店铺 writer fence 由 ConcurrentEditorGuard 唯一管理**。它只提供围栏事实；Runner 仍是唯一调度者，Dispatcher 仍是唯一写派发者。每个 ledger `BEGIN` 重新验证 fencing token，不能用启动时一次校验替代动作时复核。
...
| `ConcurrentEditorGuard`         | 🆕  | R1      | 单店铺系统内 writer fence、generation 与崩溃隔离；不拥有队列或写派发权             |
...
- 当前保守执行顺序冻结为 `video → HVD checkpoint → wholesale → HVD checkpoint → translation → HVD checkpoint → delegated-field readback`。这是 `PRODUCT_DECISION`，不是对店小秘字段依赖的伪造 `OBSERVED`；取得真实依赖证据后只能通过新合同版本改变。

#### 6.2.1 inspect effect 观测合同

`inspect` 与 `capture_preimage` 只允许四类最终裁决：`PURE_READ`、`UI_REVEAL`、`READ_ONLY_DXM_REQUEST`、`FORBIDDEN`。分类依据是实际观察事实，不是 DOM 标签、事件名称或 HTTP method：

1. 每个 inspect action 必须来自版本化 allowlist，并绑定稳定 `action_key`、`BindingRegistry` identity、`page_kind` 和声明的 `expected_effects`；未登记动作默认 `FORBIDDEN`。
2. 动作前记录 URL/page epoch、页面与商品身份、可见表单值 hash、文件选择、网络与导航基线；动作中同时观察 DOM、form value、文件、弹窗、下载、导航和全部网络请求。
3. 以观察到的最严重 effect 为 `final_effect_class`；任何未声明 effect 直接 `FORBIDDEN`。`evaluate` 默认禁止，只有经审阅的无副作用读取表达式才可作为内部 read primitive。
4. `UI_REVEAL` 只在表单值、文件、页面身份和外部 mutation 均未变化时成立。Modal 必须通过精确 reveal action 打开、精确无提交 close/cancel 关闭，并证明关闭后状态等于 preimage。
5. `READ_ONLY_DXM_REQUEST` 只由精确 method + URL + request shape + response contract allowlist 判定；GET 不天然只读，POST 也不天然是 mutation。
6. `FORM_VALUE_WRITE`、`FILE_UPLOAD`、`LONG_ACTION_DISPATCH`、`SAVE_OR_PUBLISH` 或任何意外副作用均折叠为 `FORBIDDEN`，整页写入计数必须保持 0。

```yaml
inspect_effect_receipt:
  action_key: ...
  ...
  navigation_facts: []
  unexpected_effects: []
  final_effect_class: PURE_READ | UI_REVEAL | READ_ONLY_DXM_REQUEST | FORBIDDEN
```
...
必须把“某类目有没有某个字段”与“产品阶段能否跳过”分开。11 个 Section Module 对每件商品都必须运行并产出 receipt；当前 Schema 明确不存在某类字段时，可以得到可审计的空字段结果，但这不是 `skip`。视频、批发、翻译、半托管和 rollback preparation 是产品级 `ALWAYS_ON` 阶段，不能被类目适用性关闭。
...
ContentFinalize(video) → HVD checkpoint
ContentFinalize(wholesale) → HVD checkpoint
ContentFinalize(translation) → HVD checkpoint
...
canonical_serialization:
  schema: dxm_canonical_json.v1
  encoding: UTF-8
  unicode_normalization: NFC
  object_key_order: lexicographic
  arrays: preserve_contract_order
  domain_numbers: normalized_decimal_strings
...
execution_constraints:
  contract_version: dxm_execution_constraints.v1
  max_start_age_hours: 24
  session_refresh_allowed: false
  category_drift_action: ABORT
  schema_drift_action: ABORT
  catalog_drift_action: ABORT
  draft_presence_check: REQUIRED
items:
  - queue_index: 0
    product_identity: ...
    category_id: ...
...
真实合同还必须包含逐字段来源、wire/normalized 类型、stable binding、当前值、目标值、规则版本、preimage requirement、readback expectation、页面身份、队列版本、每个 action 的 `possible_effects`、SAVE request 枚举/幂等证据策略和 canonical serialization 版本。`dxm_batch_draft_save_plan.v2` 仍是目标合同、尚未发布；R1 发布前必须先冻结上述 `execution_constraints`，不得先生成可执行 v2 再补安全字段。`execution_attempt_id` 不属于 snapshot 内容；它在每次启动时唯一创建，并绑定 task、command、ledger 和 receipt。

三层身份不可混用：

- `plan_content_sha256`：先做领域归一化，再按 `dxm_canonical_json.v1` 序列化 UTF-8 bytes 并 hash。输入白名单必须包含账号/店铺业务作用域、有序商品及全部 resolution、类目/Schema/capability/binding、五项必经阶段、两页动作与 possible effect sets、`execution_constraints`、approval policy/scope 和 expected final state；明确排除 `session_ref/session_epoch`、created/captured time、实际 approval record、snapshot instance/freeze key、attempt/command/grant/lease 和 hash 字段自身。相同业务事实必须得到相同值。
...
#### 8.2.1 Canonical serialization 与实时有效性

- ID 一律使用稳定字符串；金额、重量、尺寸使用规范十进制字符串，禁止语言运行时浮点差异。object key 按字典序；商品、步骤、SKU 等有业务顺序的数组保持冻结顺序。`null`、missing、空字符串和空数组不可互换。
- snapshot 必须记录 canonical schema/version、byte length 与 canonical bytes 的可复核摘要；实现不得只保存某语言运行时对象后声称可重算。
- Python 与 TypeScript 必须共用 golden vectors，覆盖 object key 顺序、Unicode NFC、十进制、`null`/missing、数组重排、时间污染和不同 freeze key。
- 24 小时只是额外的 task-start 上限，不替代实时检查，也不允许运营覆盖。session 变化不得自动刷新后继续旧 attempt。
- 强制复核点：task start 前；每件商品主页面首写前；每个外部 action 与 SAVE ledger `BEGIN` 前；店小秘原生门放行并精确绑定 `editFromSmt` 后；S1–S3 首写前；SECOND_SAVE 前。
- 任一复核发现草稿缺失、类目、Schema 或 catalog 漂移必须在任何后续写入前 `ABORT`。不得使用 `PROCEED_WITH_WARNING`、无限有效期、可关闭的 draft check 或人工覆盖安全约束。
...
- `resolving` 表示外部动作已派发或页面已改变，但证据 resolver 尚未闭合；UI 只能显示黄色“正在判定最终状态”，不得显示成功或失败。
- `继续`禁用；`暂停`只记录 `pause_requested`，在 resolving 闭合后的下一个安全点生效；`停止`与`急停`只记录 stop intent、阻止新命令，不能终止既有外部事实或伪造终态。
- resolver 只有在取得明确成功、明确业务拒绝，或证据期限耗尽而无法证明结果时结束；最后一种进入 `UNKNOWN` 并要求人工复核。UI 的 warning threshold 仅用于提醒，不改变机器状态，禁止把固定 60 秒当作 UNKNOWN 裁决。
- 用户可以关闭详情面板，但 task/item 的全局 warning badge 必须持续存在，且同店铺不得开始新 writer。
...
direct `editFromSmt` landing 不在当前可执行状态图中，状态固定为 `DISABLED_UNTIL_OBSERVED_EVIDENCE_VERSION`。首次观察到该路径时只记录同一 `entry_handshake_id`、SAVE1、原生门、正式 HTTPS URL、task/job/product/shop/category/page epoch 和零发布事实，随后停批人工裁决；没有版本化 `OBSERVED` Evidence Pack 前不得现场启用分支。
...
- `resolving` 期间：暂停延后到下一个安全点；继续禁用；停止/急停只记录停止意图并阻止新命令。任何控制键都不能把尚未闭合的外部事实直接改成成功、失败、cancelled 或 UNKNOWN。
...
- **R0B · 真实只读 Evidence Pack**：取得视频、批发、翻译、资质图片、类目切换、非空级联、草稿 revision checkpoint，以及半托管原生门 admitted/rejected/unknown 的脱敏证据；未实证的超时、资格规则和控件不得冻结成实现常量。
...
- 店铺/账号/会话变化使所有下游对象失效。
- 冻结 `dxm_canonical_json.v1`、不可放宽的 `dxm_execution_constraints.v1` 和同店铺 writer fence；它们均是目标合同，不得伪装成当前已实现事实。

DoD：≥3 商品、多类目相同输入可重复生成相同 `plan_content_sha256`；Python/TypeScript golden vectors 一致；每次 freeze 有唯一 snapshot instance；遗漏任一必经分区、主页面能力、binding、执行约束或当前 Schema 即 fail-closed，且不得把半托管运行时资格伪造成 freeze 时 READY。
...
- 在主页面首个 `apply` 前完成主页面全部分区和能力预检；半托管页只在店小秘原生门放行并精确绑定 `editFromSmt` 后、该页首个 `apply` 前完成 S1–S3 预检。每个 inspect action 必须来自版本化 allowlist，并用真实 before/after effect facts 判定。
...
| snapshot 超过启动上限、草稿缺失、类目/Schema/catalog 漂移时仍以 warning、人工覆盖或关闭检查继续写入 | execution constraints 反例 | P0 · 立即回退 |
...
| 未取得版本化 `OBSERVED` Evidence Pack 就启用 direct `editFromSmt` landing 分支 | 状态机/版本门禁 | P0 · 立即回退 |
...
| 仅凭 DOM 标签、事件名或 GET/POST method 判定 inspect effect，未记录 before/after 与网络事实 | effect allowlist 反例 | P0 · 立即回退 |
| reveal Modal 后未用精确无提交动作关闭并证明恢复到 preimage | browser/effect receipt 反例 | P0 · 立即回退 |
...
| Python/TypeScript 对 Unicode、十进制、null/missing 或数组业务顺序生成不同 canonical bytes/hash | 共用 golden vectors | P0 · R1 阻断 |
...
| 两个同店铺 task 同时取得 writer fence，或旧 generation/fencing token 通过 ledger BEGIN | 并发 CAS/崩溃恢复反例 | P0 · 立即回退 |
| 外部 revision/类目/Schema 漂移后仅因系统 writer fence 仍有效而继续写入 | 实时 checkpoint 反例 | P0 · 立即回退 |
...
- 视频、批发、翻译、资质图片、切类目和级联的新证据，应先形成脱敏 fixture 与事实条目，再进入 SectionAutomation Implementation。
...
#### E_CONCURRENT_* — 系统内 writer 围栏

| code | 含义 | HTTP 状态 | outcome/处置摘要 |
| --- | --- | --- | --- |
| `E_CONCURRENT_EDITOR_CONFLICT` | 同店铺已有系统内 writer，或当前 fencing token/generation 已失效 | 409 | `blocked_pre_write`；不得启动或派发新动作 |
| `E_CONCURRENT_BATCH_CONFLICT` | 队列 owner、task/attempt 或 writer fence 与当前批次不一致 | 409 | `blocked_pre_write`；人工核对孤儿围栏 |
...
| `E_DRAFT_EXTERNAL_MODIFICATION` | 实时 revision 证明商品被外部修改 | 409 | `needs_manual_review` |
| `E_DRAFT_CONCURRENT_BATCH_MODIFICATION` | 另一系统内任务改变了当前商品 revision | 409 | `needs_manual_review` |
| `E_DRAFT_SNAPSHOT_EXPIRED` | snapshot 超过 `max_start_age_hours`，不得启动旧 attempt | 409 | `blocked_pre_write` |
| `E_DRAFT_CATEGORY_DRIFT` | 当前类目与冻结 `category_id` 不一致 | 409 | `blocked_pre_write` |
| `E_DRAFT_SCHEMA_DRIFT` | 当前 Schema hash 与冻结 `schema_sha256` 不一致 | 409 | `blocked_pre_write` |
| `E_DRAFT_CATALOG_DRIFT` | 当前 catalog hash 与冻结 `catalog_sha256` 不一致 | 409 | `blocked_pre_write` |
