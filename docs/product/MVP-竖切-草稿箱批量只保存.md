...
| **批次** | 一次任务中由人工确认的 `draft` 商品 id 队列，MVP 人工验收数量至少 3 |
...
运营人员在 DXM 控制台连接一个**真实可见浏览器**会话，通过只读接口取得店铺与采集箱草稿，人工多选 **draft ≥3**，选择并确认方案快照，然后由同一受控 runner 串行执行 `batch_draft_save`：逐品在真实店小秘编辑页用 UI 填写并点击精确「保存」，通过 HVD 查看同源进度，并可开始、暂停、继续或停止；任何发布动作永久禁止。
...
- MVP 人工验收要求多选至少 3 个商品；实现与测试不得硬编码为恰好 3。
...
- [ ] 只读接口拉取真实店铺与 `pageList(draft)`，人工多选 ≥3；
...
- 前端多选真实 draft id，至少能支持 ≥3，生成可审阅任务输入；
...
- 真实账号人工批准后，至少 3 个 draft Path A 各自三铁证；
- 任何单品缺证据不得计入成功；
...
3. 人工勾选至少 3 个真实 draft；核对每个 id 均来自本次读回。
...
11. 对至少 3 个成功商品逐一核对保存回包、页面成功态、独立未发布证明。
