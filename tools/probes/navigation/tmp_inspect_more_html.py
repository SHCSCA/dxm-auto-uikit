...
with sync_playwright() as p:
    ...
    print(json.dumps(page.evaluate(r'''() => {
      const more = Array.from(document.querySelectorAll('*')).find(el => (el.innerText||el.textContent||'').replace(/\s+/g,'').trim()==='更多');
      if (!more) return {found:false};
      const chain=[]; let p=more; for(let i=0;i<5 && p;i++,p=p.parentElement){ chain.push({tag:p.tagName, cls:p.className||'', html:p.outerHTML.slice(0,500)}); }
