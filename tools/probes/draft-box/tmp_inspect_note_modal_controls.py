...
with sync_playwright() as p:
    ...
    print(json.dumps(page.evaluate(r'''() => {
      const modal=Array.from(document.querySelectorAll('[role="dialog"], .ant-modal-wrap, .ant-modal')).find(el => (el.innerText||el.textContent||'').includes('备注 内容：'));
      if(!modal) return {found:false};
      const controls=Array.from(modal.querySelectorAll('*')).map(el=>{
        const st=getComputedStyle(el), r=el.getBoundingClientRect();
        if(st.display==='none'||st.visibility==='hidden'||parseFloat(st.opacity||'1')===0||r.width<5||r.height<5) return null;
        return {tag:el.tagName, txt:(el.innerText||el.textContent||el.getAttribute('value')||'').replace(/\s+/g,' ').trim().slice(0,100), placeholder:el.getAttribute('placeholder'), cls:String(el.className||''), style:el.getAttribute('style')||'', rect:{x:r.x,y:r.y,w:r.width,h:r.height}, html:el.outerHTML.slice(0,300)};
      }).filter(Boolean);
      return {found:true, modalText:(modal.innerText||modal.textContent||'').replace(/\s+/g,' ').trim(), controls:controls.slice(0,200)};
