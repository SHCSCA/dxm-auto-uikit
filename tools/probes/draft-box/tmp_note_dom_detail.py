...
with sync_playwright() as p:
    ...
    print(json.dumps(page.evaluate(r'''() => {
      ...
      const textareas = Array.from(modal.querySelectorAll('textarea,input')).map(el => { const r=el.getBoundingClientRect(); return {tag:el.tagName, type:el.getAttribute('type'), placeholder:el.getAttribute('placeholder'), cls:String(el.className||''), val:el.value||'', rect:{x:r.x,y:r.y,w:r.width,h:r.height}, html:el.outerHTML.slice(0,200)}; });
      const colorish = Array.from(modal.querySelectorAll('*')).map(el => { const st=getComputedStyle(el), r=el.getBoundingClientRect(); const bg=st.backgroundColor; const border=st.borderColor; const txt=(el.innerText||el.textContent||'').replace(/\s+/g,' ').trim(); if(r.width<8||r.height<8) return null; if((bg && bg!='rgba(0, 0, 0, 0)' && bg!='rgb(255, 255, 255)') || txt in ['红色','蓝色','绿色']) return {tag:el.tagName, txt, cls:String(el.className||''), bg, border, rect:{x:r.x,y:r.y,w:r.width,h:r.height}, html:el.outerHTML.slice(0,200)}; return null; }).filter(Boolean);
      return {found:true, modalText:(modal.innerText||modal.textContent||'').replace(/\s+/g,' ').trim(), textareas, colorish: colorish.slice(0,80)};
