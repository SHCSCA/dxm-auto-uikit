...
with sync_playwright() as p:
    ...
    print('box', claim.bounding_box())
    try:
        claim.click(timeout=3000)
        print('clicked normal')
    except Exception as e:
        print('normal click err', e)
    page.wait_for_timeout(2500)
    print(json.dumps(page.evaluate(r'''() => ({
      dialogTexts: Array.from(document.querySelectorAll('[role="dialog"], .ant-modal-wrap, .ant-select-dropdown, .ant-dropdown')).map(el => ((el.innerText||el.textContent||'').replace(/\s+/g,' ').trim())).slice(0,10),
      active: document.activeElement ? {tag: document.activeElement.tagName, cls: document.activeElement.className || '', text:(document.activeElement.innerText||document.activeElement.textContent||'').replace(/\s+/g,' ').trim()} : null,
