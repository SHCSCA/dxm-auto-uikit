...
with sync_playwright() as p:
    ...
    print(json.dumps(page.evaluate(r'''() => {
      const modalEl = Array.from(document.querySelectorAll('[role="dialog"], .ant-modal-wrap')).find(el => (el.innerText||el.textContent||'').includes('备注 内容：'));
      if (!modalEl) return {found:false};
      const body = modalEl.querySelector('.ant-modal-body') || modalEl;
      const els = Array.from(body.querySelectorAll('*')).map(el => {
        const st=getComputedStyle(el), r=el.getBoundingClientRect();
        if(st.display==='none'||st.visibility==='hidden'||parseFloat(st.opacity||'1')===0||r.width<5||r.height<5) return null;
        return {tag:el.tagName, txt:(el.innerText||el.textContent||el.getAttribute('value')||'').replace(/\s+/g,' ').trim().slice(0,120), placeholder:el.getAttribute('placeholder'), cls:String(el.className||''), style:el.getAttribute('style')||'', rect:{x:r.x,y:r.y,w:r.width,h:r.height}};
      }).filter(Boolean);
      return {found:true, text:(modalEl.innerText||modalEl.textContent||'').replace(/\s+/g,' ').trim(), els};
