...
- **扩展 E3 集合未能收集：** 复跑 `test_e3_batch_draft_save_gates.py` 等更大集合时，当前机器已有运行时占用仓库 `data` 的 lease，测试在 import 阶段以 `RuntimeLeaseConflictError` fail-closed；未删除或接管该运行时数据，故不能把这次集合结果算作通过。
